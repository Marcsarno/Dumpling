import { Script, Vec3, Application, FILLMODE_NONE, RESOLUTION_AUTO, Color, Entity, SHADOW_PCF3_32F, Asset, BoundingBox, StandardMaterial, Quat, AnimTrack, AnimData, AnimCurve, INTERPOLATION_LINEAR, Texture, ADDRESS_REPEAT, FILTER_LINEAR_MIPMAP_LINEAR, TONEMAP_LINEAR, PROJECTION_ORTHOGRAPHIC, AnimStateGraph, math, SEMANTIC_BLENDINDICES, SEMANTIC_BLENDWEIGHT, Mat4, KEY_D, KEY_RIGHT, KEY_A, KEY_LEFT, KEY_W, KEY_UP, KEY_S, KEY_DOWN, Vec2, Keyboard, CULLFACE_NONE, Mesh, TorusGeometry, MeshInstance, TONEMAP_ACES, PRIMITIVE_TRIANGLES, BLEND_NORMAL } from '../playcanvas-stable.min.mjs';
...
async function captureWorld(app, house, props, loop, editor) {
    ...
    const serialize = (r)=>{
        ...
        return {
            ...
            name: e.name,
            model: r.model,
            modelMaterials: r.model ? e.findComponents("render").flatMap((r2)=>r2.meshInstances.map((mi)=>{
            ...
            position: e.getLocalPosition().toArray(),
            rotation: e.getLocalEulerAngles().toArray(),
            scale: e.getLocalScale().toArray(),
            scope: e.parent.name,
            enabled: e.enabled,
            render: e.render ? {
                type: e.render.type,
                castShadows: e.render.castShadows
            } : void 0,
            material: m ? {
                name: m.name,
                diffuse: [
            ...
            normalization: r.model ? e.children[0].getLocalScale().toArray() : void 0,
            offset: r.model ? e.children[0].getLocalPosition().toArray() : void 0
        };
    };
    const data = {
        ...
        rooms: rooms.map((r)=>({
                name: r.root.name,
                walkable: r.walkable,
                obstacles: r.obstacles.map((b)=>({
                        center: b.center.toArray(),
                        half: b.halfExtents.toArray()
                    }))
            })),
        interactions: props.interactions.filter((i)=>i.kind === "place" || i.kind === "daily" && !/play-lilah|wipe-|vacuum-|put-tool/.test(i.id)).map((i)=>({
                id: i.id,
                anchor: i.anchor.toArray(),
                marker: i.marker.toArray(),
                placement: i.placement
            })),
        stores: loop.stores.map((s)=>({
                name: s.root.name,
                sites: s.sites.map((t)=>({
                        id: t.id,
                        anchor: t.anchor.toArray(),
                        marker: t.marker.toArray()
                    })),
                exit: s.exitAnchor.toArray()
            }))
    };
    ...
    if (editor) {
        ...
        for (const [key, x, z] of [
            ...
            if (e) registerPropSpace(key, e, new Vec3(x, 0, z));
        }
        ...
        const bounds = new BoundingBox(new Vec3(), new Vec3(0.5, 0.5, 0.5));
        ...
        for (const r of rooms)for (const n of app.root.findByTag("walkable:" + r.root.name)){
            ...
            if (r.walkable?.[index]) Object.assign(r.walkable[index], {
                minX: b.center.x - b.halfExtents.x,
                maxX: b.center.x + b.halfExtents.x,
                minZ: b.center.z - b.halfExtents.z,
                maxZ: b.center.z + b.halfExtents.z
            });
        }
        ...
        for (const s of loop.stores)for(let i = 0; i < s.sites.length; i++){
            ...
            for (const child of [
                ...
                const p = child.getLocalPosition().clone().sub(new Vec3(origin[0], 0, origin[2]));
        ...
        for (const s of loop.stores){
            for(let i = 0; i < s.sites.length; i++){
                ...
                if (anchor) s.glows[i].setLocalPosition(anchor.getLocalPosition().clone().add(new Vec3(0, 0.027, 0)));
            }
        ...
        HousePath = class {
            free(x, z) {
                for (const a of [
                    ...
                    z - this.radius,
                    z + this.radius
                ])if (!this.house.walkable?.some((r)=>a >= r.minX && a <= r.maxX && b >= r.minZ && b <= r.maxZ)) return false;
            ...
            route(start, end) {
                ...
                for(let dx = -1; dx <= 1; dx++)for(let dz = -1; dz <= 1; dz++){
                    ...
                    open.push({
                        ...
                        z,
                        g,
                        f: g + p.distance(end) / step
                    });
                }
                for(let iterations = 0; open.length && iterations < 5e3; iterations++){
                    open.sort((a, b)=>a.f - b.f);
                    const current = open.shift(), p = new Vec3(current.x * step, 0, current.z * step);
                ...
                while(finish){
                    path.unshift(new Vec3(finish.x * step, 0, finish.z * step));
        ...
        DogRoaming = class {
            update(dt, active, people) {
                ...
                if (!this.route.length) {
                    const candidates = this.spots.map((s, i)=>({
                            i,
                            p: new Vec3(s[0], 0, s[1])
                        })).filter((s)=>s.i !== this.last && this.path.free(s.p.x, s.p.z) && s.p.distance(p) > 1);
                    const choice = candidates[Math.floor(Math.random() * candidates.length)];
                    if (choice) {
                        this.route = this.path.route(new Vec3(p.x, 0, p.z), choice.p);
                        this.last = choice.i;
                    }
                    if (!this.route.length) this.wait = 3;
                    return;
                }
                const next = this.route[0], delta = new Vec3(next.x - p.x, 0, next.z - p.z), distance = delta.length();
                if (distance < 0.015) {
                    this.route.shift();
            ...
            constructor(house, root){
                ...
                _define_property(this, "route", []);
                _define_property(this, "wait", 4);
                _define_property(this, "blocked", 0);
                _define_property(this, "last", -1);
                _define_property(this, "spots", [
                    [
                        4.7,
                        6
                    ],
...
var init_HouseMusic = __esm({
    "src/ui/HouseMusic.ts" () {
        ...
        tracks = {
            home: {
                file: "Squishy home clean.mp3",
                name: "Home \xB7 morning & night"
            },
            chores: {
                file: "Squishy Home clean v2.mp3",
                name: "Home \xB7 after-school chores"
            },
            school: {
                file: "Squishy school trading.mp3",
                name: "School trading"
            },
            shop: {
                file: "Squishy shopping.mp3",
                name: "Shopping \xB7 1"
            },
            shop2: {
                file: "Squishy shopping v2.mp3",
                name: "Shopping \xB7 2"
            }
        };
        HouseMusic = class {
            ...
            snapshot() {
                return {
                    ...
                    muted: this.muted,
                    track: tracks[this.current].name,
                    requested: tracks[this.desired].name,
                    source: decodeURIComponent(this.audio.src),
            ...
            constructor(){
                _define_property(this, "audio", new Audio());
                _define_property(this, "unlocked", false);
                _define_property(this, "wanted", false);
                _define_property(this, "muted", false);
                _define_property(this, "button", document.createElement("button"));
                _define_property(this, "abort", new AbortController());
                _define_property(this, "current", "home");
                _define_property(this, "desired", "home");
                _define_property(this, "lastStore", "");
                _define_property(this, "secondStore", false);
                _define_property(this, "gap", 0);
                _define_property(this, "pending", false);
                try {
...
async function loadSquishyArt(app) {
    const load = (name)=>new Promise((resolve, reject)=>{
            const asset = new Asset(name, "container", {
                url: assetUrl(`${"/"}assets/squishies/${name}.glb`)
            });
    ...
    const texture = (name)=>new Promise((resolve, reject)=>{
            const asset = new Asset(name, "texture", {
                url: assetUrl(`${"/"}assets/squishies/materials/${name}.png`)
            });
            asset.once("load", ()=>resolve(asset.resource));
            asset.once("error", reject);
            app.assets.add(asset);
    ...
    const [bao, steamer, shelf, color, surface] = await Promise.all([
        load("bao-squishy"),
        load("bamboo-steamer"),
        load("bamboo-steamer-shelf"),
        texture("satin-color"),
        texture("satin-surface")
    ]);
    loaded.set(app, {
        bao,
        steamer,
...
function squishyModel(app, data) {
    const model = loaded.get(app).bao.instantiateRenderEntity({
        castShadows: true
    });
    for (const name of [
        "leaf",
        ...
        "star",
        "crown"
    ])model.findByName("Accessory_" + name).enabled = data.accessory === name;
    for (const side of [
        "L",
        "R"
    ]){
...
function steamerModel(app, small = false) {
    return loaded.get(app)[small ? "shelf" : "steamer"].instantiateRenderEntity({
        castShadows: true
    });
}
        ...
        HOUSE_ROOMS = [
            {
                id: "bedroom",
                name: "Bedroom",
                title: "A little room of her own.",
                minX: -3.3,
                maxX: 3.3,
                minZ: -3.6,
                maxZ: 3.6
            },
            {
                id: "hall",
                name: "Landing",
                title: "Little everyday arrivals.",
                minX: 3.3,
                maxX: 6.5,
                minZ: 0.4,
                maxZ: 3.6
            ...
            },
            {
                id: "nursery",
                name: "Lilah\u2019s room",
                title: "Little dreams, big adventures.",
                minX: 6.5,
                maxX: 11,
                minZ: -3.6,
                maxZ: 3.6
            },
            {
                id: "marc-bedroom",
                name: "Marc\u2019s room",
                title: "Dad\u2019s little quiet corner.",
                minX: 6.5,
                maxX: 11,
                minZ: 3.6,
                maxZ: 13.2
...
function createBedroom(app) {
    ...
    const obstacles = [];
    const block = (x, z, w, d)=>obstacles.push(new BoundingBox(new Vec3(x, 0.7, z), new Vec3(w / 2, 1.4, d / 2)));
    shape("Room foundation", "box", [
        0,
        -0.22,
        0
    ], [
    ...
    for(let z = -3.3; z < 3.5; z += 0.45)shape("Plank joint", "box", [
        0,
        0.03,
        z
    ], [
    ...
    for(let i = 0; i < 15; i++){
        ...
        for(let x = -2.8 + i % 3 * 0.75; x < 3.3; x += 2.25)shape("Staggered plank end", "box", [
            x,
            0.031,
            z
        ], [
    ...
    shape("Folded lavender blanket", "box", [
        -2.05,
        0.895,
        -0.73
    ], [
    ...
    const pillow = shape("Pillow", "sphere", [
        -2.05,
        0.88,
        -2.5
    ], [
    ...
    for (const [x, z] of [
        ...
        for(let i = 0; i < 5; i++){
            ...
            const petal = shape("Daisy petal", "sphere", [
                x + Math.sin(a) * 0.2,
                0.078,
                z + Math.cos(a) * 0.2
            ], [
    ...
    for(let i = 0; i < 3; i++)shape("Stacked book", "box", [
        0.81,
        0.83 + i * 0.09,
        -2.99
    ], [
    ...
    function plant(x, y, z, scale = 1) {
        ...
        for(let i = 0; i < 5; i++){
            const a = i * Math.PI * 2 / 5;
            const leaf = shape("Plant leaf", "sphere", [
                x + Math.sin(a) * 0.16 * scale,
                y + 0.51 * scale,
                z + Math.cos(a) * 0.16 * scale
            ], [
                0.2 * scale,
                0.56 * scale,
                0.15 * scale
    ...
    for(let i = 0; i < 6; i++){
        const a = i * Math.PI / 3;
        shape("Picture petal", "sphere", [
            -1 + Math.sin(a) * 0.16,
            2.08 + Math.cos(a) * 0.16,
            -3.445
        ], [
            0.2,
            0.2,
        ...
        HouseArt = class {
            add(pack, name, position, size, yaw = 0, dimension = "height", colors = {}, exterior = pack === "nature", pitch = 0, finish = "natural") {
                ...
                if (!this.assets.has(key)) this.assets.set(key, new Promise((resolve, reject)=>{
                    ...
                    const asset = new Asset(key, "container", {
                        url: assetUrl(path)
                    }, {}, containerOptions(path));
                    asset.once("load", ()=>resolve(asset.resource));
                    asset.once("error", reject);
                ...
                const task = this.assets.get(key).then((resource)=>{
                    ...
                    for (const render of renderers)for (const mesh of render.meshInstances){
                        ...
                        if (!this.materials.has(paletteKey)) {
                            ...
                            if (color) material2.diffuse = new Color().fromString(color);
            ...
            snapshot() {
                return {
                    ...
                    ].filter((m)=>m.diffuseMap?.name.startsWith("Subtle")).map((m)=>({
                            name: m.name,
                            texture: m.diffuseMap.name
                        }))
                };
            }
            constructor(app, root, surfaces){
                ...
                _define_property(this, "assets", /* @__PURE__ */ new Map());
                _define_property(this, "materials", /* @__PURE__ */ new Map());
                _define_property(this, "pending", []);
                _define_property(this, "group", void 0);
                _define_property(this, "loaded", 0);
                _define_property(this, "errors", []);
                _define_property(this, "lampMaterials", []);
                this.app = app;
                this.root = root;
    ...
    "src/game/HouseLighting.ts" () {
        ...
        HouseLighting = class {
            ...
            constructor(app, root, bedside, importedShades){
                _define_property(this, "importedShades", void 0);
                _define_property(this, "amount", 0);
                _define_property(this, "shadedCount", -1);
                _define_property(this, "lights", []);
                _define_property(this, "shades", void 0);
                ...
                addLight("Bedside lamp light", [
                    -0.53,
                    1.39,
                    -2.83
                ], 2.4, 3.8);
                addLight("Reading floor lamp light", [
                    5.65,
                    1.58,
                    7.7
                ], 2.3, 3.5);
                addLight("Lilah nursery lamp", [
                    7.1,
                    1.48,
                    -1.15
                ], 2.3, 6);
                addLight("Marc bedside lamp", [
                    10.6,
                    0.94,
                    4.5
                ], 2.3, 6);
                addLight("Marc reading lamp", [
                    10.55,
                    1.48,
                    8
                ], 2.3, 6);
                for (const [name, x, y, z, leftWall, floorLamp] of [
                    ...
                    if (!floorLamp) {
                        ...
                        shape(name + " sconce glass", "sphere", [
                            x + (leftWall ? 0.08 : 0),
                            y,
                            z + (leftWall ? 0 : 0.08)
                        ], leftWall ? [
                    ...
                    addLight(name + " fixture light", [
                        x + (leftWall ? 0.22 : 0),
                        y,
                        z + (leftWall || floorLamp ? 0 : 0.22)
                    ], 2.1, 5.5);
                }
                const fill = new Entity("Warm interior bounce", app);
                root.addChild(fill);
                fill.addComponent("light", {
                    type: "directional",
                    color: new Color(1, 0.88, 0.74),
                    intensity: 0,
                    castShadows: false
                });
                fill.light.mask = 1;
...
function createHouse(app) {
    ...
    const root = new Entity("Maple cottage", app);
    app.root.addChild(root);
    ...
    const block = (x, z, w, d)=>obstacles.push(new BoundingBox(new Vec3(x, 0.7, z), new Vec3(w / 2, 1.4, d / 2)));
    const box = (name, x, y, z, w, h, d, mat = m.trim)=>shape(name, "box", [
            x,
            y,
            z
        ], [
    ...
    const oval = (name, x, y, z, w, h, d, mat = m.rug)=>shape(name, "cylinder", [
            x,
            y,
            z
        ], [
    ...
    function furniture(name, x, z, size, yaw = 0, dimension = "height", foot, y = 0.027, colors = {}, finish = "natural") {
        art.add("furniture", name, [
            x,
            y,
            z
        ], size, yaw, dimension, colors, exterior, 0, finish);
    ...
    function wall(axis, fixed, from, to, height = 0.48, mat = m.sideWall) {
        const gaps = HOUSE_DOORS.filter((d)=>d.axis === axis && Math.abs((axis === "x" ? d.z : d.x) - fixed) < 0.01).map((d)=>({
                a: (axis === "x" ? d.x : d.z) - d.width / 2,
                b: (axis === "x" ? d.x : d.z) + d.width / 2
    ...
    for (const door of HOUSE_DOORS){
        ...
        for (const side of [
            -1,
            1
        ])box("Door frame cutaway", door.x + (door.axis === "x" ? side * door.width / 2 : 0), 0.46, door.z + (door.axis === "z" ? side * door.width / 2 : 0), 0.15, 0.92, 0.15, m.trim);
    }
    function windowOnLeft(z, w = 1.8) {
        ...
        for (const dz of [
            -w / 2 - 0.14,
            w / 2 + 0.14
        ])box("Linen curtain", -3.02, 1.82, z + dz, 0.16, 1.63, 0.27, m.pinkLight);
    }
    ...
    shape("Door brass handle", "sphere", [
        -3.075,
        1,
        7.92
    ], [
    ...
    for (const z of [
        7.7,
        8.8
    ])box("Entry door casing", -3.1, 1.17, z, 0.19, 2.34, 0.09, m.trim);
    box("Entry lintel", -3.1, 2.32, 8.25, 0.19, 0.09, 1.19, m.trim);
    furniture("benchCushion", 6.02, 3.05, 0.8, -90, "width", [
        0.5,
        0.8
    ], 0.027, {
        carpet: "#b9a7cd"
    });
    furniture("coatRackStanding", 3.8, 1, 1.55, 0, "height", [
        0.4,
        0.4
    ]);
    furniture("sideTableDrawers", 5.8, 1.1, 0.72, 0, "height", [
        0.75,
        0.45
    ]);
    furniture("plantSmall1", 5.8, 1.1, 0.28, 0, "height", void 0, 0.76);
    furniture("lampRoundFloor", 6.1, 3.28, 1.8, 0, "height", [
        0.35,
        0.35
    ]);
    oval("Landing rug", 4.7, 0.054, 2.15, 1.75, 0.025, 1.55, m.rug);
    box("Mail tray", 5.55, 0.8, 1.1, 0.42, 0.08, 0.3, m.wood);
    furniture("loungeSofa", -2.3, 6.05, 3.05, 90, "width", [
        1.2,
        3.05
    ]);
    furniture("tableCoffee", -0.48, 6.3, 1.3, 0, "width", [
        1.3,
        0.75
    ]);
    ...
    for (const z of [
        4.7,
        7.7
    ])box("Rug woven border", -0.9, 0.067, z, 3.2, 4e-3, 0.035, m.trim);
    furniture("cabinetTelevision", 2, 4.2, 1.8, 0, "width", [
        1.8,
        0.6
    ]);
    furniture("televisionModern", 2, 4.2, 1.28, 0, "width", void 0, 0.68, {
        metal: "#718d91"
    });
    furniture("loungeChairUpright", 4.5, 7.35, 1, -35, "height", [
        1.1,
        1.1
    ], 0.027, {
        carpet: "#d9b7c3"
    });
    furniture("lampRoundFloor", 5.65, 7.7, 1.8, 0, "height", [
        0.35,
        0.35
    ]);
    furniture("sideTable", 5.65, 6.6, 0.55, 0, "height", [
        0.5,
        0.5
    ]);
    furniture("radio", 5.65, 6.6, 0.32, 0, "width", void 0, 0.61);
    furniture("bookcaseOpenLow", 5.95, 4.9, 1.45, -90, "width", [
        0.6,
        1.45
    ]);
    furniture("books", 5.95, 4.9, 0.65, -90, "width", void 0, 1.03);
    furniture("pottedPlant", 5.9, 8.75, 1.1, 0, "height", [
        0.5,
        0.5
    ]);
    ...
    furniture("kitchenCabinetDrawer", -2.72, 10.25, 1, 90, "height", [
        0.95,
        1.1
    ]);
    furniture("kitchenSink", -2.72, 11.45, 1, 90, "height", [
        0.95,
        1.2
    ]);
    furniture("kitchenStove", -2.72, 12.65, 1, 90, "height", [
        0.95,
        1.1
    ]);
    furniture("kitchenFridge", -2.65, 14.55, 1.85, 90, "height", [
        1.05,
        1.05
    ]);
    ...
    furniture("tableRound", 0.55, 13.85, 1.75, 0, "width", [
        1.65,
        1.65
    ]);
    ...
    ])furniture("chairCushion", x, z, 0.82, yaw, "height", [
        0.55,
        0.55
    ]);
    furniture("plantSmall1", 0.55, 13.85, 0.28, 0, "height", void 0, 0.95);
    furniture("trashcan", 1.95, 10.3, 0.6, 0, "height", [
        0.4,
        0.4
    ]);
    box("Kitchen woven runner", -1.65, 0.05, 11.35, 0.65, 0.025, 2.7, m.mint);
    furniture("washer", 3.35, 10.2, 1.02, 0, "height", [
        1.05,
        0.95
    ]);
    furniture("dryer", 3.25, 12.4, 1.02, 90, "height", [
        1.05,
        0.95
    ]);
    ...
    for(let i = 0; i < 3; i++)box("Folded towels", 5.6, 0.98 + i * 0.07, 12.65, 0.55, 0.055, 0.32, [
        m.blue,
        m.pinkLight,
        m.mint
    ][i]);
    furniture("plantSmall2", 6.1, 12.65, 0.3, 0, "height", void 0, 0.95);
    furniture("lampRoundFloor", 6.1, 11.45, 1.8, 0, "height", [
        0.35,
        0.35
    ]);
    furniture("bathroomSink", 4.12, -3.08, 0.9, 0, "height", [
        1,
        0.7
    ]);
    furniture("bathroomMirror", 4.12, -3.49, 0.9, 0, "height", void 0, 1.35, {
        metal: "#a5c6cc"
    });
    furniture("bathtub", 5.75, -2.12, 2.15, 90, "width", [
        0.94,
        2.15
    ]);
    furniture("toilet", 5.98, -0.32, 0.8, -90, "height", [
        0.8,
        0.58
    ]);
    ...
    for (const z of [
        -0.85,
        -0.2
    ])box("Towel rail leg", 3.62, 0.44, z, 0.055, 0.88, 0.055, m.wood);
    ...
    art.add("nursery", "crib", [
        9.8,
        0.035,
        -1.7
    ], 1.25, 0, "height", {
        "_crayfishdiffuse": "#e9dbc5",
        "03___Default": "#c5d4cf",
        "02___Default": "#ecd59d"
    });
    block(9.8, -1.7, 1.6, 2);
    furniture("cabinetBedDrawer", 7.3, -2.85, 0.88, 0, "height", [
        0.9,
        0.7
    ], 0.027, {
        wood: "#e8e5df"
    }, "paint");
    art.add("furniture", "pillowLong", [
        7.3,
        0.96,
        -3.03
    ], 0.68, 0, "width", {
    ...
    furniture("bookcaseOpenLow", 10.55, 1.55, 1.1, -90, "height", [
        0.6,
        1.5
    ], 0.027, {
        wood: "#e8e5df"
    ...
    furniture("loungeChairRelax", 7.15, -0.1, 0.83, 45, "height", [
        0.85,
        0.85
    ], 0.027, {
        carpet: "#b9c5ac"
    });
    furniture("lampRoundFloor", 7.1, -1.15, 1.65, 0, "height", [
        0.3,
        0.3
    ]);
    ...
    furniture("bedDouble", 9.3, 5.15, 1.9, 0, "width", [
        1.95,
        2.4
    ], 0.035, {
        wood: "#566d7d",
    ...
    for (const x of [
        8,
        10.6
    ])furniture("cabinetBedDrawerTable", x, 4.5, 0.55, 0, "height", [
        0.55,
        0.5
    ], 0.027, {
        wood: "#ede9df",
    ...
    furniture("lampRoundTable", 10.6, 4.5, 0.42, 0, "height", void 0, 0.6, {
        metal: "#b79c70"
    });
    ...
    furniture("bookcaseClosedDoors", 7.02, 8.45, 1.7, 90, "height", [
        0.75,
        1.55
    ], 0.027, {
        wood: "#657c89",
    ...
    furniture("desk", 8.45, 12.65, 1.8, 180, "width", [
        1.8,
        0.75
    ], 0.027, {
        wood: "#ece8de",
    ...
    furniture("chairModernFrameCushion", 8.45, 11.55, 0.88, 180, "height", [
        0.65,
        0.65
    ], 0.027, {
        metal: "#526777",
    ...
    furniture("loungeDesignChair", 10.05, 9.15, 0.95, -45, "height", [
        1.1,
        1.1
    ], 0.027, {
        carpetBlue: "#b97f65",
    ...
    furniture("tableCoffeeGlassSquare", 9.9, 10.6, 0.62, 0, "width", [
        0.62,
        0.62
    ], 0.027, {
        metal: "#566a76",
    ...
    furniture("lampRoundFloor", 10.55, 8, 1.65, 0, "height", [
        0.3,
        0.3
    ], 0.027, {
        metal: "#b59b76"
    });
    furniture("pottedPlant", 10.5, 12.4, 0.95, 0, "height", [
        0.5,
        0.5
    ]);
    furniture("cabinetBedDrawer", -2.72, 15.65, 1, 90, "height", [
        0.95,
        0.9
    ]);
    furniture("kitchenMicrowave", -2.72, 15.65, 0.29, 90, "height", void 0, 1.035);
    furniture("pillowLong", -2.3, 6.8, 0.66, 90, "width", void 0, 0.62, {
        carpet: "#b5c9bc"
    });
    exterior = true;
    ...
    ])box("Raised flower border", x, -0.07, z, w, 0.11, d, soil);
    const treePositions = [
        [
            -6.2,
            0,
    ...
    treePositions.forEach(([x, , z], i)=>art.add("nature", i % 2 ? "tree_oak" : "tree_detailed", [
            x,
            -0.14,
            z
        ], 3 + i % 3 * 0.45, i * 57));
    for(let i = 0; i < 28; i++){
        ...
        art.add("nature", "plant_bushDetailed", [
            x,
            -0.1,
            z
        ], 0.48 + i % 3 * 0.08, i * 41);
        art.add("nature", i % 2 ? "flower_purpleA" : "flower_yellowC", [
            x + 0.35,
            -0.07,
            z + 0.25
        ], 0.26 + i % 3 * 0.03, i * 23);
    }
    for(let i = 0; i < 30; i++)art.add("nature", "grass_large", [
        -11 + i % 10 * 2.4,
        -0.14,
        i < 10 ? -6.3 : i < 20 ? 18.4 : 21.5
    ], 0.15 + i % 3 * 0.03, i * 17);
    for(let z = -6; z < 20; z += 2)art.add("nature", "fence_planksDouble", [
        15,
        -0.14,
        z
    ], 0.82, 90);
    for(let x = -10; x < 15; x += 2)art.add("nature", "fence_planksDouble", [
        x,
        -0.14,
        -6
    ], 0.82, 0);
    for(let x = -4.7; x < 15; x += 2)art.add("nature", "fence_planksDouble", [
        x,
        -0.14,
        20
    ], 0.82, 0);
    ...
    "src/game/IsometricCamera.ts" () {
        ...
        IsometricCamera = class {
            ...
            constructor(app, existing){
                ...
                _define_property(this, "basePosition", new Vec3(6, 14, 18.9));
                _define_property(this, "baseTarget", new Vec3(0, 0.8, 0.9));
                _define_property(this, "zoomFactor", void 0);
                ...
                if (existing) {
                    ...
                    this.baseTarget.copy(existing.getPosition()).add(existing.forward.clone().mulScalar(new Vec3(6, 14, 18.9).distance(new Vec3(0, 0.8, 0.9))));
                }
    ...
    "src/components/CharacterAnimator.ts" () {
        CharacterAnimator = class {
            ...
            attach(model, animations, manifest, scale) {
                const tracks2 = new Map(animations.map((track)=>[
                        track.name,
                        track
                    ]));
...
function createCleanupProps(app, room) {
    ...
    const items = [
        item("teddy", "Teddy", "\u{1F9F8}", [
            -0.7,
            0.11,
            1.55
        ]),
        item("shirt", "Shirt", "\u{1F455}", [
            -1.7,
            0.12,
            0.55
        ]),
        item("book", "Book", "\u{1F4D8}", [
            0.25,
            0.12,
            -0.8
        ]),
    ...
    const hamperPosition = [
        -2.55,
        0,
        1
    ];
    fixed("Laundry hamper", "cylinder", [
        -2.55,
        0.33,
        1
    ], [
    ...
    const crayonMess = new Entity("Scattered crayons", app);
    ...
    [
        ...
        const pen = crayons("Scattered crayon", "cylinder", [
            -0.33 + i * 0.2,
            0.015,
            i % 2 * 0.17
        ], [
    ...
    [
        ...
        m.yellow,
        m.mint
    ].forEach((mat, i)=>tidy("Tidy crayon", "cylinder", [
            (i % 2 - 0.5) * 0.07,
            0.22,
            (Math.floor(i / 2) - 0.5) * 0.07
        ], [
            0.045,
            0.25,
            0.045
    ...
    const interactions = items.map((item2)=>({
            ...
            kind: "pickup",
            item: item2.id,
            anchor: new Vec3(...item2.home),
            marker: new Vec3(item2.home[0], item2.home[1] + (item2.id === "vacuum" ? 0.82 : 0.62), item2.home[2]),
            range: 0.85
        }));
    interactions.push({
        id: "toy-chest",
        ...
        item: "teddy",
        task: "teddy",
        anchor: new Vec3(2.48, 0, -0.79),
        marker: new Vec3(2.48, 1.45, -1.55),
        range: 1.1,
        placement: [
            2.48,
            0.85,
        ...
        item: "shirt",
        task: "shirt",
        anchor: new Vec3(-2.55, 0, 1),
        marker: new Vec3(-2.55, 1.04, 1),
        range: 1.05,
        placement: [
            -2.55,
            0.69,
        ...
        item: "book",
        task: "book",
        anchor: new Vec3(1.12, 0, -2.55),
        marker: new Vec3(1.12, 2.35, -3.05),
        range: 1.05,
        placement: [
            1.51,
            0.79,
        ...
        kind: "crayons",
        task: "crayons",
        anchor: new Vec3(1.6, 0, 1.05),
        marker: new Vec3(2.15, 1.68, 1.04),
        range: 0.88
    }, {
        id: "dirt",
        name: "Dirt pile",
        ...
        item: "vacuum",
        task: "dirt",
        anchor: new Vec3(-1.4, 0, 2.65),
        marker: new Vec3(-1.4, 0.55, 2.65),
        range: 1
    });
    const reset = ()=>{
...
async function importProp(app, parent, name, height, idle = false, rotation = [
    0,
    0,
    0
]) {
    const asset = new Asset(name, "container", {
        url: assetUrl(`${"/"}assets/pets/${name}.glb`)
    });
    await new Promise((resolve, reject)=>{
        asset.once("load", resolve);
        asset.once("error", reject);
        ...
        DogAnimator = class {
            ...
            snapshot() {
                return {
                    state: this.state,
                    speed: this.speed,
                    time: this.model.anim.baseLayer.activeStateCurrentTime,
                    position: this.root.getPosition().toArray(),
                    height: this.height,
                    asset: "sunny-pup.glb"
                };
            }
            constructor(root, model){
                ...
                _define_property(this, "previous", new Vec3());
    ...
    "src/game/PetCleanup.ts" () {
        ...
        PetCleanup = class {
            ...
            get hint() {
                ...
                return ({
                    tool: "\u{1F43E} Pick up the scooper by the landing, then find the poop.",
                    scoop: "\u{1F944} Carry the scooper to the dog poop, then tap Scoop.",
                    flush: "\u{1F6BD} Take the loaded scooper to the bathroom toilet and tap Flush.",
                    wash: "\u{1FAE7} Almost done! Go to the bathroom sink and wash your hands."
                })[this.stage];
            }
            snapshot() {
                return {
                    ...
                    loaded: this.loaded,
                    errors: this.errors,
                    poopVisible: this.poop.enabled,
                    poopParent: this.poop.parent?.name,
                    washing: this.bubbles.enabled,
                    flushing: this.water.enabled
                };
            }
            constructor(app, props){
                _define_property(this, "props", void 0);
                _define_property(this, "stage", "tool");
                _define_property(this, "active", false);
                _define_property(this, "loaded", false);
                _define_property(this, "errors", []);
                _define_property(this, "tool", void 0);
                _define_property(this, "poop", void 0);
                _define_property(this, "dog", void 0);
                _define_property(this, "dogAnimator", void 0);
                _define_property(this, "bubbles", void 0);
                _define_property(this, "water", void 0);
                _define_property(this, "flushStart", 0);
                this.props = props;
                const tool = new Entity("Pooper scooper", app);
                ...
                this.water = shape("Toilet flushing water", "cylinder", [
                    5.78,
                    0.48,
                    -0.36
                ], [
                ...
                props.interactions.push({
                    ...
                    item: "scooper",
                    task: "pet-care",
                    anchor: new Vec3(...this.tool.home),
                    marker: new Vec3(3.8, 1, 3),
                    range: 0.85,
                    available: (carried)=>this.loaded && this.stage === "tool" && !carried
                }, {
                    id: "scoop-poop",
                    name: "Dog poop",
                    icon: "\u{1F4A9}",
                    ...
                    task: "pet-care",
                    item: "scooper",
                    anchor: new Vec3(3.7, 0, 5.3),
                    marker: new Vec3(3.7, 0.55, 5.3),
                    range: 0.9,
                    available: (carried)=>this.stage === "scoop" && carried === "scooper"
                }, {
                    id: "flush-poop",
                    name: "Toilet",
                    icon: "\u{1F6BD}",
                    ...
                    task: "pet-care",
                    item: "scooper",
                    anchor: new Vec3(5.15, 0, -0.38),
                    marker: new Vec3(5.87, 1, -0.38),
                    range: 0.85,
                    available: (carried)=>this.stage === "flush" && carried === "scooper"
                }, {
                    id: "wash-hands",
                    ...
                    task: "pet-care",
                    anchor: new Vec3(4.12, 0, -2.35),
                    marker: new Vec3(4.1, 1.45, -3.08),
                    range: 0.9,
                    available: (carried)=>this.stage === "wash" && !carried
                });
                this.reset(false);
            }
        };
...
var DUST_LOCATIONS, SPILL_LOCATIONS, DAILY_TASKS, DailyClock;
    ...
    "src/systems/DailyClock.ts" () {
        ...
        DAILY_TASKS = {
            ...
            afternoon: [
                ...
                {
                    id: "spill",
                    name: "Wipe kitchen spill",
                    icon: "\u{1F9FB}"
                },
                {
                    id: "laundry-clothes",
                    name: "Put laundry in washer",
                    icon: "\u{1F455}"
                }
            ],
            night: [
        ...
        DailyClock = class {
            newDay(day) {
                ...
                return {
                    ...
                    dust,
                    schoolSeconds: 0,
                    petTask: this.random() < 0.5 ? "feed-dog" : "pet-care",
                    spillSite: Math.floor(this.random() * SPILL_LOCATIONS.length),
                    sideTask: [
                        "laundry-clothes",
                        "living-toy",
                        "kitchen-dish"
            ...
            get tasks() {
                ...
                return [
                    ...tasks.filter((t)=>t.id !== "dust-2" && t.id !== "laundry-clothes"),
                    this.state.sideTask === "living-toy" ? {
                        id: "living-toy",
                        name: "Put toys away",
                        icon: "\u{1F9F8}"
                    } : this.state.sideTask === "kitchen-dish" ? {
                        id: "kitchen-dish",
                        name: "Take dish to sink",
                        icon: "\u{1F37D}"
                    } : {
                        id: "laundry-clothes",
                        name: "Put laundry in washer",
                    ...
                    this.state.petTask === "feed-dog" ? {
                        id: "feed-dog",
                        name: "Fill puppy\u2019s bowl",
                        icon: "\u{1F43E}"
                    } : {
                        id: "pet-care",
                        name: "Scoop \xB7 flush \xB7 wash",
            ...
            constructor(random = Math.random, saved){
                _define_property(this, "random", void 0);
                _define_property(this, "state", void 0);
                this.random = random;
                this.state = this.newDay(1);
                const s = saved;
                if (s?.version === 1 && Number.isInteger(s.day) && s.day > 0 && [
                    "morning",
                    "school",
                    "afternoon",
                    "night"
                ].includes(s.phase) && Number.isFinite(s.minutes) && s.minutes >= 420 && s.minutes <= 1260 && Array.isArray(s.done) && s.done.every((x)=>typeof x === "string") && Array.isArray(s.dust) && s.dust.length === 3 && new Set(s.dust).size === 3 && s.dust.every((x)=>Number.isInteger(x) && x >= 0 && x < DUST_LOCATIONS.length) && [
                    "eggs",
                    "spill",
                    "cook",
                    "serve",
                    "done"
                ].includes(s.breakfast) && [
                    true,
                    false,
                    null
                ].includes(s.eggDrop) && Number.isFinite(s.schoolSeconds)) this.state = structuredClone(s);
            }
        };
    }
});
...
var init_LilahMesses = __esm({
    "src/game/LilahMesses.ts" () {
        ...
        LilahMesses = class {
            ...
            save(records2) {
                try {
                    localStorage.setItem("dumpling.editorMigration.lilah.v1", JSON.stringify({
                        day: this.day,
                        messes: records2
                    }));
                    return true;
            ...
            refresh() {
                this.tasks = this.records.map((_, i)=>({
                        id: "lilah-mess-" + i,
                        name: [
                            "Lilah\u2019s toys",
                            "Lilah\u2019s spill",
            ...
            get completed() {
                return this.records.flatMap((m, i)=>m.done ? [
                        "lilah-mess-" + i
                    ] : []);
            }
            needs(kind) {
            ...
            complete(id, actor = "arianna") {
                ...
                const records2 = this.records.map((m, j)=>j === i ? {
                        ...m,
                        done: true,
                        cleanedBy: actor
                    } : m);
            ...
            snapshot() {
                return {
                    day: this.day,
                    messes: this.records.map((m, i)=>({
                            ...m,
                            id: "lilah-mess-" + i,
                            kind: this.kinds[i],
                            visible: this.roots[i].enabled
                        })),
                    budget: 3
                };
            }
            constructor(app, parent, props, active){
                ...
                _define_property(this, "day", 0);
                _define_property(this, "records", []);
                _define_property(this, "roots", []);
                _define_property(this, "tasks", []);
                _define_property(this, "kinds", [
                    "toys",
                    "spill",
                    "crumbs"
                ]);
                _define_property(this, "onClean", (_actor)=>{});
                this.props = props;
                this.active = active;
                const colors = [
                    "#db9fc9",
                ...
                for(let i = 0; i < 3; i++){
                    ...
                    for(let n = 0; n < (i === 0 ? 4 : 8); n++)shape(this.kinds[i], i === 0 ? "box" : "sphere", [
                        ...
                        0.035,
                        0.08
                    ], i === 0 ? colors[n] : i === 1 ? juice : crumb, false);
...
var init_DailyLife = __esm({
    "src/game/DailyLife.ts" () {
        ...
        DailyLife = class {
            ...
            get completed() {
                return [
                    ...this.clock.state.done,
                    ...this.lilahMesses.completed
                ];
            }
            save() {
                try {
            ...
            refresh() {
                ...
                this.plate.entity.setLocalPosition(...s.breakfastAtTable === false ? this.plate.home : propTuple("dining", [
                    0.55,
                    0.975,
                    13.28
                ]));
                ...
                for (const item of [
                    ...
                    this.towel,
                    this.egg
                ])if (item.entity.parent !== this.root) {
                ...
                this.outfit.entity.setLocalPosition(...s.phase === "night" ? propTuple("bed", [
                    -1.4,
                    0.91,
                    -0.65
                ]) : this.outfit.home);
            ...
            update(now, held, busy) {
                ...
                if (this.eggFall) {
                    ...
                    this.egg.entity.setLocalPosition(...propTuple("stove", [
                        -2.63 + 1.13 * t,
                        1.12 * (1 - t * t),
                        12.65 - 0.1 * t
                    ]));
            ...
            perform(target, carry) {
                ...
                switch(target.id){
                    ...
                    case "serve-breakfast":
                        carry.release(this.root, propTuple("dining", [
                            0.55,
                            0.975,
                            13.28
                        ]));
            ...
            snapshot() {
                return {
                    ...
                    canShop: this.clock.canShop,
                    held: this.held,
                    dirt: this.dust.map((e)=>({
                            visible: e.enabled,
                            position: e.getPosition().toArray(),
                            scale: e.getLocalScale().toArray()
                        })),
                    eggSpill: this.eggSpill.enabled,
                    lilah: this.lilahMesses.snapshot()
                };
            }
            constructor(app, props, house){
                ...
                _define_property(this, "active", false);
                _define_property(this, "root", void 0);
                _define_property(this, "outfit", void 0);
                _define_property(this, "towel", void 0);
                _define_property(this, "egg", void 0);
                _define_property(this, "plate", void 0);
                _define_property(this, "dust", []);
                _define_property(this, "spill", void 0);
                _define_property(this, "eggSpill", void 0);
                _define_property(this, "lilahMesses", void 0);
                _define_property(this, "lilahTarget", void 0);
                _define_property(this, "lilahAvailable", false);
                _define_property(this, "onPlayLilah", ()=>{});
                _define_property(this, "taskCache", []);
                _define_property(this, "taskKey", "");
                _define_property(this, "cooked", void 0);
                _define_property(this, "bubbles", void 0);
                _define_property(this, "wipingPaper", void 0);
                _define_property(this, "dogFood", void 0);
                _define_property(this, "eggFall", 0);
                _define_property(this, "held", null);
                _define_property(this, "last", 0);
                _define_property(this, "lastSave", 0);
                _define_property(this, "phase", "");
                _define_property(this, "onPhaseChange", ()=>{});
                _define_property(this, "onReward", ()=>{});
                _define_property(this, "onStore", ()=>{});
                this.props = props;
                let saved;
                try {
                ...
                this.root = new Entity("Daily routines", app);
                ...
                this.lilahTarget = {
                    id: "play-lilah",
                    name: "Play with Lilah",
                    actionLabel: "Play with Lilah",
                    icon: "\u{1F495}",
                    kind: "daily",
                    anchor: new Vec3(),
                    marker: new Vec3(),
                    range: 1.1,
                    duration: 650,
                    available: (h)=>this.active && this.lilahAvailable && !h
                };
                ...
                this.outfit = item("daily-outfit", "Clothes", "\u{1F455}", [
                    -0.7,
                    0.7,
                    3.05
                ]);
                ...
                art.add("furniture", "sideTableDrawers", [
                    -0.7,
                    0.025,
                    3.05
                ], 0.85, 180);
                void art.finish();
                house.obstacles.push(new BoundingBox(new Vec3(-0.7, 0.5, 3.05), new Vec3(0.35, 1, 0.35)));
                this.towel = item("paper-towel", "Paper towel", "\u{1F9FB}", [
                    -2.65,
                    1.02,
                    10.5
                ]);
                void importProp(app, this.towel.entity, "paper", 0.23);
                this.egg = item("breakfast-egg", "Egg", "\u{1F95A}", [
                    -2.68,
                    1.15,
                    14.55
                ]);
                const eggShape = primitives(app, this.egg.entity);
                eggShape("Egg placeholder", "sphere", [
                    0,
                    0.1,
                    0
                ], [
                ...
                this.cooked = new Entity("Cooked breakfast", app);
                this.root.addChild(this.cooked);
                this.cooked.setLocalPosition(...propTuple("stove", [
                    -2.63,
                    1.12,
                    12.65
                ]));
                const food = primitives(app, this.cooked);
                food("Egg white", "sphere", [
                    0,
                    0,
                    0
                ], [
                ...
                this.plate = item("breakfast-plate", "Egg on a plate", "\u{1F373}", [
                    -2.63,
                    1.13,
                    12.65
                ]);
                ...
                plate("Breakfast plate rim", "cylinder", [
                    0,
                    0,
                    0
                ], [
                ...
                plate("Breakfast plate", "cylinder", [
                    0,
                    0.021,
                    0
                ], [
                ...
                const makeSpill = (name, position)=>{
                    ...
                    for(let i = 0; i < 7; i++)s("Puddle", "sphere", [
                        Math.sin(i * 2) * 0.22,
                        0.01,
                        Math.cos(i * 2) * 0.16
                    ], [
                        ...
                        0.022,
                        0.3
                    ], i === 0 ? m.yellow : m.trim, false);
                    return root;
                };
                this.spill = makeSpill("Kitchen spill", [
                    0.1,
                    0.04,
                    11.1
                ]);
                this.eggSpill = makeSpill("Dropped egg", [
                    -1.5,
                    0.04,
                    12.55
                ]);
                ...
                for(let k = 0; k < 3; k++){
                    const root = new Entity("Random dirt " + k, app);
                    this.root.addChild(root);
                    const s = primitives(app, root);
                    for(let i = 0; i < 10; i++)s("Dust fleck", "sphere", [
                        Math.sin(i * 2) * 0.26,
                        0.016,
                        Math.cos(i * 2) * 0.24
                    ], [
                ...
                this.bubbles = new Entity("Routine feedback", app);
                this.root.addChild(this.bubbles);
                const b = primitives(app, this.bubbles);
                for(let i = 0; i < 6; i++)b("Foam", "sphere", [
                    Math.sin(i) * 0.12,
                    i * 0.025,
                    Math.cos(i) * 0.1
                ], [
                ...
                this.wipingPaper = shape("Paper wiping the floor", "box", [
                    0,
                    0.09,
                    0
                ], [
                ...
                const target = (id, name, icon, anchor, marker, available, duration = 650, task, hold = false, mess)=>{
                    props.interactions.push({
                        ...
                        kind: "daily",
                        actionLabel: name,
                        anchor: new Vec3(...anchor),
                        marker: new Vec3(...marker),
                        range: 1,
                        task,
                        duration,
                        hold,
                ...
                shape("Puppy bowl inside", "cylinder", [
                    4.95,
                    0.185,
                    8.55
                ], [
                ...
                this.dogFood = shape("Puppy kibble", "sphere", [
                    4.95,
                    0.19,
                    8.55
                ], [
                    ...
                    0.055,
                    0.33
                ], material("Kibble", "#b58655"));
                target("feed-dog", "Fill puppy\u2019s bowl", "\u{1F43E}", [
                    4.8,
                    0,
                    8.8
                ], [
                    ...
                    0.2,
                    8.55
                ], (h)=>!h && phase() === "afternoon" && this.clock.state.petTask === "feed-dog" && notDone("feed-dog"), 1400, "feed-dog");
                target("daily-teeth", "Brush teeth", "\u{1FAA5}", [
                    4.12,
                    0,
                    -2.35
                ], [
                    ...
                    1.15,
                    -3.03
                ], (h)=>!h && (phase() === "morning" || phase() === "night") && notDone("teeth"), 1800, "teeth");
                target("choose-clothes", "Choose clothes", "\u{1F455}", [
                    -0.7,
                    0,
                    2.4
                ], [
                    ...
                    0.8,
                    3.05
                ], (h)=>!h && phase() === "morning" && notDone("outfit"), 0);
                target("get-dressed", "Get dressed", "\u{1F455}", [
                    -0.85,
                    0,
                    -0.65
                ], [
                    ...
                    0.9,
                    -0.65
                ], (h)=>h === "daily-outfit" && phase() === "morning", 850, "outfit");
                target("night-clothes", "Pick up clothes", "\u{1F455}", [
                    -0.85,
                    0,
                    -0.65
                ], [
                    ...
                    0.9,
                    -0.65
                ], (h)=>!h && phase() === "night" && notDone("outfit"), 0);
                target("clothes-drawer", "Put clothes inside", "\u{1F455}", [
                    -0.7,
                    0,
                    2.4
                ], [
                    ...
                    0.8,
                    3.05
                ], (h)=>h === "daily-outfit" && phase() === "night", 0, "outfit");
                target("take-egg", "Take an egg", "\u{1F95A}", [
                    -1.8,
                    0,
                    14.55
                ], [
                    ...
                    1.1,
                    14.55
                ], (h)=>!h && phase() === "morning" && this.clock.state.breakfast === "eggs", 0);
                target("crack-egg", "Crack egg", "\u{1F95A}", [
                    -1.8,
                    0,
                    12.65
                ], [
                ...
                target("cook-egg", "Cook breakfast", "\u{1F373}", [
                    -1.8,
                    0,
                    12.65
                ], [
                    ...
                    1.15,
                    12.65
                ], (h)=>!h && phase() === "morning" && this.clock.state.breakfast === "cook", 2200);
                target("take-breakfast", "Carry breakfast", "\u{1F373}", [
                    -1.8,
                    0,
                    12.65
                ], [
                    ...
                    1.15,
                    12.65
                ], (h)=>!h && phase() === "morning" && this.clock.state.breakfast === "serve" && this.clock.state.breakfastAtTable === false, 0);
                target("serve-breakfast", "Put breakfast on table", "\u{1F37D}", [
                    0.55,
                    0,
                    11.9
                ], [
                    ...
                    0.9,
                    13.15
                ], (h)=>h === "breakfast-plate", 0);
                target("eat-breakfast", "Eat breakfast", "\u{1F37D}", [
                    0.55,
                    0,
                    11.9
                ], [
                    ...
                    0.9,
                    13.15
                ], (h)=>!h && phase() === "morning" && this.clock.state.breakfast === "serve" && this.clock.state.breakfastAtTable !== false, 4200, "breakfast");
                target("take-towel", "Take paper towel", "\u{1F9FB}", [
                    -1.8,
                    0,
                    10.5
                ], [
                    ...
                    1.15,
                    10.5
                ], (h)=>!h && (this.clock.state.breakfast === "spill" || phase() === "afternoon" && notDone("spill") || this.lilahMesses.needs("spill")), 0);
                target("wipe-egg", "Wipe dropped egg", "\u{1F9FB}", [
                    -1.5,
                    0,
                    12.55
                ], [
                    ...
                    0.12,
                    12.55
                ], (h)=>h === "paper-towel" && this.clock.state.breakfast === "spill", 1200, void 0, true, this.eggSpill);
                target("wipe-spill", "Wipe spill", "\u{1F9FB}", [
                    0.1,
                    0,
                    11.1
                ], [
                    ...
                    0.12,
                    11.1
                ], (h)=>h === "paper-towel" && phase() === "afternoon" && notDone("spill"), 1200, "spill", true, this.spill);
                const vacuum = props.items.find((i)=>i.id === "vacuum");
                target("daily-vacuum", "Pick up vacuum", "\u2726", vacuum.home, [
                    vacuum.home[0],
                    1,
                    vacuum.home[2]
                ], (h)=>!h && (phase() === "afternoon" && this.clock.tasks.some((t)=>t.id.startsWith("dust-") && notDone(t.id)) || this.lilahMesses.needs("crumbs")), 0);
                for(let i = 0; i < 3; i++)target("vacuum-" + i, "Hold to vacuum", "\u2726", [
                    0,
                    0,
                    0
                ], [
                    ...
                    0.3,
                    0
                ], (h)=>h === "vacuum" && phase() === "afternoon" && this.clock.tasks.some((t)=>t.id === "dust-" + i) && notDone("dust-" + i), 1150, "dust-" + i, true, this.dust[i]);
                target("put-tool-away", "Put tool away", "\u21A9", [
                    0,
                    0,
                    0
                ], [
                    ...
                    0.8,
                    0
                ], (h)=>h === "vacuum" || h === "paper-towel", 0);
                target("bedtime-book", "Read a bedtime book", "\u{1F4D8}", [
                    -0.85,
                    0,
                    -0.8
                ], [
                    ...
                    0.9,
                    -0.8
                ], (h)=>!h && phase() === "night" && notDone("read"), 1600, "read");
                target("school-door", "Go to school", "\u{1F392}", [
                    -2.35,
                    0,
                    8.2
                ], [
                    ...
                    1.1,
                    8.2
                ], (h)=>!h && this.clock.schoolDue, 0);
                target("shop-door", "Choose a store", "\u{1F6CD}", [
                    -2.35,
                    0,
                    8.2
                ], [
                    ...
                    1.1,
                    8.2
                ], (h)=>!h && this.clock.canShop && (phase() === "night" || this.clock.ready), 0);
                target("sleep", "Go to bed", "\u{1F319}", [
                    -0.85,
                    0,
                    -1.6
                ], [
        ...
        RoundMesses = class {
            ...
            apply(mode, random = Math.random) {
                ...
                const choose = (id, pool)=>{
                    const candidates = pool.map(([x, z])=>new Vec3(x, 0, z)).filter((p2)=>this.planner.free(p2.x, p2.z) && used.every((q)=>q.distance(p2) > 0.65) && this.planner.route(new Vec3(0, 0, 0.9), p2).length);
                ...
                for (const item of this.props.items){
                    ...
                    if (mode === "day" || mode === "pet" || !item.entity.enabled || [
                        "vacuum",
                        "scooper"
            ...
            constructor(props, house){
                ...
                _define_property(this, "previous", /* @__PURE__ */ new Map());
                _define_property(this, "lastTasks", "");
                this.props = props;
...
function createHouseProps(app, house) {
    ...
    function pair(id, name, icon, home, destination, anchor, placement, visual, range = 0.95) {
        ...
        if (visual === "toy") {
            ...
            shape("Folded fabric", "box", [
                ...
                0.12,
                0.33
            ], id.includes("towel") ? m.blue : m.pink);
            shape("Fabric stripe", "box", [
                0,
                0.134,
                0
            ], [
            ...
            shape("Toiletry bottle", "cylinder", [
                0,
                0.17,
                0
            ], [
            ...
            for(let i = 0; i < 3; i++)shape("Crumpled paper", "box", [
                (i - 1) * 0.09,
                0.07 + i * 0.025,
                0
            ], [
        ...
        props.interactions.push({
            ...
            item: id,
            task: id,
            anchor: new Vec3(...home),
            marker: new Vec3(home[0], home[1] + 0.6, home[2]),
            range: 0.85
        }, {
            id: `place-${id}`,
            name: destination,
            ...
            task: id,
            anchor: new Vec3(...anchor),
            marker: new Vec3(placement[0], placement[1] + 0.65, placement[2]),
            range,
            placement,
            placedStyle: id === "bath-towel" ? "hang" : [
                "kitchen-trash",
                "laundry-clothes"
            ].includes(id) ? "hide" : void 0
        });
    }
    pair("hall-shoes", "Shoes", "\u{1F45F}", [
        4.45,
        0.09,
        2.25
    ], "Shoe bench", [
    ...
    pair("living-toy", "Living-room toy", "\u{1F9F8}", [
        1,
        0.09,
        5.25
    ], "Toy basket", [
    ...
    pair("kitchen-dish", "Dish", "\u{1F37D}", [
        -0.55,
        0.09,
        11.6
    ], "Kitchen sink", [
    ...
    pair("laundry-clothes", "Dirty laundry", "\u{1F455}", [
        4.3,
        0.09,
        10.9
    ], "Washer", [
    ...
    pair("laundry-clean", "Clean laundry", "\u25A4", [
        5.85,
        0.48,
        10.5
    ], "Folding counter", [
    ...
    "src/ui/CleanupFeedback.ts" () {
        ...
        CleanupFeedback = class {
            ...
            constructor(app, camera, layer, targets){
                ...
                _define_property(this, "layer", void 0);
                _define_property(this, "markers", []);
                _define_property(this, "mesh", void 0);
                _define_property(this, "glow", void 0);
                _define_property(this, "screen", new Vec3());
                _define_property(this, "popups", []);
                this.camera = camera;
                this.layer = layer;
                this.mesh = Mesh.fromGeometry(app.graphicsDevice, new TorusGeometry({
                    tubeRadius: 0.055,
                    ringRadius: 0.43,
                    segments: 32,
                    sides: 8
                ...
                for (const target of targets){
                    ...
                    ring.addComponent("render", {
                        meshInstances: [
                            new MeshInstance(this.mesh, this.glow)
                        ],
                        castShadows: false,
    ...
    "src/ui/CleanupHUD.ts" () {
        CleanupHUD = class {
            ...
            update(mission, carry, focus, busy, progress, animation = null, petHint) {
                ...
                if (focus) {
                    title = focus.actionLabel ?? ({
                        pickup: "Pick up",
                        place: "Put away",
                        crayons: "Tidy up",
                        vacuum: "Hold to clean",
                        pet: "Action",
                        daily: "Action"
                    })[focus.kind];
                    detail = focus.name;
            ...
            constructor(button2, replay){
                ...
                _define_property(this, "clock", document.querySelector("#mission-clock"));
                _define_property(this, "allowance", document.querySelector("#allowance"));
                _define_property(this, "count", document.querySelector("#task-count"));
                _define_property(this, "hint", document.querySelector("#cleanup-hint"));
                _define_property(this, "actionTitle", document.querySelector("#action-title"));
                _define_property(this, "actionDetail", document.querySelector("#action-detail"));
                _define_property(this, "actionIcon", document.querySelector("#action-icon"));
                _define_property(this, "tasks", document.querySelector("#task-list"));
                _define_property(this, "announcement", document.querySelector("#cleanup-announcement"));
                _define_property(this, "dialog", document.querySelector("#results"));
                _define_property(this, "abort", new AbortController());
                _define_property(this, "taskDefinitions", null);
                this.button = button2;
...
function bedEntry(start, startYaw, crib, progress) {
    const end = crib ? new Vec3(9.8, 0.09, -1.7) : new Vec3(-2.05, 0.09, -1.85), yaw = crib ? 90 : 0;
    const keys = crib ? [
        {
            t: 0,
            p: start,
        ...
        {
            t: 0.2,
            p: new Vec3(8.55, 0.09, -1.3),
            h: 0.12,
            y: 90
        },
        {
            t: 0.43,
            p: new Vec3(8.76, 0.09, -1.5),
            h: 1.22,
            y: 90
        },
        {
            t: 0.65,
            p: new Vec3(9.45, 0.09, -1.7),
            h: 1.22,
            y: 90
        },
        {
        ...
        {
            t: 0.2,
            p: new Vec3(-1, 0.09, -1.75),
            h: 0.08,
            y: 90
        },
        {
            t: 0.43,
            p: new Vec3(-1.35, 0.09, -1.85),
            h: 0.91,
            y: 90
        },
        {
            t: 0.65,
            p: new Vec3(-1.8, 0.09, -1.85),
            h: 0.91,
            y: 35
        },
        {
    ...
    return {
        position: new Vec3().lerp(a.p, z.p, u),
        height: a.h + (z.h - a.h) * u,
        yaw: a.y + delta * u
    };
}
    ...
    "src/game/CleanupGame.ts" () {
        ...
        CleanupGame = class {
            ...
            configure(mode) {
                ...
                const tasks = mode === "day" ? this.props.daily.tasks : mode === "pet" ? PET_TASKS : mode === "bedroom" ? TASKS : mode === "house" ? HOUSE_TASKS : [
                    ...TASKS,
                    ...HOUSE_TASKS.filter((task)=>task.id !== "book"),
                    ...EXTRA_HOUSE_TASKS,
                    ...PET_TASKS
                ];
            ...
            perform(target) {
                ...
                if (target.kind === "daily") {
                    ...
                    if (target.duration === 0) {
                        ...
                        if (target.id === "eat-breakfast") {
                            this.seatReturn = this.character.player.getPosition().clone();
                            this.controller.reset();
                            this.character.player.setPosition(propPoint("dining", new Vec3(0.55, 0.09, 12.49)));
                            this.character.visual.setLocalEulerAngles(0, propYaw("dining", 0), 0);
                            this.character.animator.setCarrying(false);
                            this.character.animator.setWorkClip("EatSit", propPoint("dining", new Vec3(0.55, 1, 13.85)));
                            return;
                        }
                ...
                if (target.kind === "pickup") {
                    ...
                    this.activity = {
                        target,
                        start: now,
                        duration: target.kind === "pet" ? 1600 : target.kind === "vacuum" ? 1150 : 450
                    };
            ...
            snapshot() {
                return {
                    ...
                    items: this.props.items.map((item)=>({
                            id: item.id,
                            position: item.entity.getPosition().toArray(),
                            home: item.home
                        })),
                    targets: this.props.interactions.map((target)=>({
                            id: target.id,
                            position: target.anchor.toArray(),
                            range: target.range
                        }))
                };
            }
            ...
            constructor(app, character, props, camera, resetMovement, controller, interactionCamera){
                _define_property(this, "character", void 0);
                _define_property(this, "props", void 0);
                _define_property(this, "resetMovement", void 0);
                _define_property(this, "controller", void 0);
                _define_property(this, "interactionCamera", void 0);
                _define_property(this, "roundId", saveId());
                _define_property(this, "onFinished", ()=>{});
                _define_property(this, "beforeReplay", ()=>true);
                _define_property(this, "onReplay", ()=>{});
                _define_property(this, "mode", "day");
                _define_property(this, "mission", new MissionSystem());
                _define_property(this, "carry", void 0);
                _define_property(this, "interactions", void 0);
                _define_property(this, "action", void 0);
                _define_property(this, "feedback", void 0);
                _define_property(this, "hud", void 0);
                _define_property(this, "activity", null);
                _define_property(this, "progress", 0);
                _define_property(this, "finishedHandled", false);
                _define_property(this, "celebrationStarted", false);
                _define_property(this, "aligning", null);
                _define_property(this, "seatReturn", null);
                _define_property(this, "bedYaw", 0);
                _define_property(this, "press", ()=>{
                    const now = performance.now();
                    this.mission.tick(now);
                    this.refreshFocus();
                ...
                _define_property(this, "cancelHold", ()=>{
...
function rollDumpling(random = randomUnit) {
    ...
    if (![
        draw,
        choice
    ].every((value)=>Number.isFinite(value) && value >= 0 && value < 1)) throw new Error("Random values must be in [0, 1).");
...
function boardPool(collection, random = Math.random) {
    const eligible = [
        .../* @__PURE__ */ new Set([
            ...DUMPLINGS.filter((d)=>collection[d.id] > 0).map((d)=>d.id),
            ...POP_STARTERS
        ])
    ];
    ...
    const choose = (pool, start, owned)=>{
        ...
        for(let i = start; i < eligible.length; i++)if (pool.every((id)=>visuallyDistinct(id, eligible[i]))) choose([
            ...pool,
            eligible[i]
        ], i + 1, owned + (collection[eligible[i]] > 0 ? 1 : 0));
    };
    choose([], 0, 0);
    return best;
}
    ...
    "src/data/squishyPop.ts" () {
        ...
        PopBoard = class {
            newPiece() {
                return {
                    id: ++this.serial,
                    kind: this.pool[Math.floor(this.random() * this.pool.length)]
                };
            }
            adjacent(a, b) {
            ...
            affected(chain, finale = false) {
                ...
                for (const index of removed){
                    ...
                    if (!power) continue;
                    const targets = power === "rainbow" ? finale ? this.pieces.flatMap((p, i)=>p.kind === piece.kind ? [
                            i
                        ] : []) : chain : this.pieces.flatMap((_, i)=>Math.abs(i % 6 - index % 6) <= (power === "mega" ? 2 : 1) && Math.abs(Math.floor(i / 6) - Math.floor(index / 6)) <= (power === "mega" ? 2 : 1) ? [
                            i
                        ] : []);
            ...
            preview() {
                const valid = this.chain.length >= 3 || this.chain.length === 1 && [
                    "bomb",
                    "mega"
                ].includes(this.pieces[this.chain[0]].power ?? "");
                return {
                    valid,
                    created: powerForChain(this.chain.length),
                    ...this.affected(valid ? this.chain : [])
                };
            }
            finale(now = 0) {
                const powers = this.pieces.flatMap((p, i)=>p.power ? [
                        i
                    ] : []);
                this.cancel();
            ...
            release(now = 0) {
                ...
                this.cancel();
                const tapPower = chain.length === 1 && [
                    "bomb",
                    "mega"
                ].includes(this.pieces[chain[0]].power ?? "");
            ...
            resolve(chain, now, finale = false) {
                ...
                if (created) {
                    ...
                    this.pieces[at] = {
                        ...this.newPiece(),
                        kind: this.pieces[at].kind,
                        power: created
                    };
                    removed.delete(at);
                }
                for(let column = 0; column < 6; column++){
                    const kept = this.pieces.map((piece, index)=>({
                            piece,
                            index
                        })).filter((x)=>x.index % 6 === column && !removed.has(x.index));
                    for(let row = 5; row >= 0; row--){
                        const old = kept.pop(), to = row * 6 + column, piece = old?.piece ?? this.newPiece();
                        this.pieces[to] = piece;
                        moves.push({
                            piece,
                            from: old?.index ?? (row - 6) * 6 + column,
                            to
                        });
                    }
                }
                ...
                if (shuffled) {
                    moves.length = 0;
                    this.pieces.forEach((piece, to)=>moves.push({
                            piece,
                            from: before.indexOf(piece.id),
                            to
                        }));
                }
                return {
                    chain: finale ? 0 : chain.length,
                    cleared,
                    moves,
                    score,
                    label: finale ? "LAST LITTLE POPS!" : chain.length >= 10 ? "SUPER SQUISH!" : chain.length >= 7 ? "GREAT!" : chain.length >= 5 ? "NICE!" : "POP!",
                    shuffled,
                    created,
                    activated,
                    effects,
            ...
            constructor(pool, collection = {}, random = Math.random){
                ...
                _define_property(this, "random", void 0);
                _define_property(this, "pieces", []);
                _define_property(this, "chain", []);
                _define_property(this, "score", 0);
                _define_property(this, "bestChain", 0);
                _define_property(this, "pops", 0);
                _define_property(this, "shuffles", 0);
                _define_property(this, "frenzyMeter", 0);
                _define_property(this, "frenzyUntil", 0);
                _define_property(this, "lastStrong", -100);
                _define_property(this, "serial", 0);
                this.pool = pool;
                ...
                this.pieces = Array.from({
                    length: POP_RULES.columns * POP_RULES.rows
                }, ()=>this.newPiece());
                this.ensurePlayable();
            }
        };
...
function createLevelBoard(level, collection, random = Math.random) {
    ...
    [
        ...
        16,
        17
    ].slice(0, level.board.openingChain).forEach((i)=>board.pieces[i].kind = board.pool[0]);
...
function createHuntDay(day, random = randomUnit) {
    ...
    for (const store of STORES){
        ...
        for(let i = 0; i < quantity; i++){
            if (i < 6) slots.push({
                site: sites[i],
                series: assortment[i % assortment.length],
                remaining: 1,
                discovered: false
            });
...
function rollSeries(series, store, random = randomUnit) {
    ...
    const rarity = weighted(Object.keys(store.odds).filter((r)=>pool.some((d)=>d.rarity === r)).map((r)=>({
            value: r,
            weight: store.odds[r]
        })), random);
...
var init_hunt = __esm({
    "src/data/hunt.ts" () {
        ...
        HUNT_RULES = {
            closingMinute: 1140,
            minimumSearchMinutes: 18,
            bagLimit: 3,
            inspectMilliseconds: 650
        };
        SERIES = [
            {
                id: "garden",
...
function valueFor(id, trader, series) {
    ...
    if (trader === "rarity") return ({
        Common: 1,
        Rare: 4,
        Epic: 10,
        Legendary: 24
    })[d.rarity];
    if (trader === "series") return ({
        Common: 1,
        Rare: 2,
        Epic: 4,
        Legendary: 7
    })[d.rarity] * (SERIES.find((s)=>s.id === series)?.items?.includes(id) ? 4 : 1);
    return ({
        Common: 1,
        Rare: 2,
        Epic: 3,
        Legendary: 5
    })[d.rarity] * (cuteIds.includes(id) || d.accessory === "bow" ? 4 : 1);
}
...
function createTradingDay(day) {
    ...
    for (const [i, trader] of TRADERS.entries()){
        ...
        traders[trader.id] = {
            stock,
            offer: stock.slice(0, i + 1),
            asks: 0,
            revision: 0,
            done: false,
            message: "Here\u2019s what I brought. What would you trade?"
        };
    }
    return {
        day,
        series: SERIES[(day - 1) % SERIES.length].id,
        traders
    };
}
function negotiate(day, trader, give) {
    ...
    const add = extras.find((id)=>npc.offer.length < 3 && offered >= offerValue([
            ...npc.offer,
            id
        ], trader, day.series));
    if (add) {
        ...
        TRADERS = [
            {
                ...
                icon: "\u2726",
                color: "#bca4e4",
                hint: "Rare finds make my day. A legendary? Even better!"
            },
            {
                id: "series",
                name: "Remy",
                ...
                icon: "\u273F",
                color: "#a9cfb5",
                hint: "I\u2019m filling the gaps in my series!"
            },
            {
                id: "cute",
                name: "Poppy",
                title: "Cute Collector",
                icon: "\u2661",
                color: "#edb3cc",
                hint: "Pink, purple, and little bows. Those are my favorites!"
            }
        ];
        cuteIds = [
            "rosie",
...
function parse(raw) {
    ...
    if (value.prizes && (!Number.isSafeInteger(value.prizes.day) || value.prizes.day < 1 || !Array.isArray(value.prizes.sold) || value.prizes.sold.some((s)=>![
            "0",
            "1",
            "2",
            "3"
        ].includes(s)) || new Set(value.prizes.sold).size !== value.prizes.sold.length)) throw Error("Prize shelf could not be read.");
    ...
    if (value.pop?.levels) {
        ...
        if (typeof levels !== "object" || Array.isArray(levels) || Object.values(levels).some((l)=>!l || typeof l.completed !== "boolean" || ![
                l.stars,
                l.bestScore,
                l.bestChain,
                l.attempts
            ].every((n)=>Number.isSafeInteger(n) && n >= 0) || l.stars > 3 || l.completed !== l.stars > 0)) throw Error("Squishy Pop levels could not be read.");
    }
    return value;
}
...
var init_ProgressStore = __esm({
    "src/systems/ProgressStore.ts" () {
        ...
        ProgressStore = class {
            ...
            /** Developer tools use the same read/validate/write transaction as gameplay. */ developerEdit(change) {
            ...
            completePopRound(id, score, attempt) {
                ...
                return this.commit((data)=>{
                    var _data;
                    const p = (_data = data).pop ?? (_data.pop = {
                        tickets: 0,
                        bestScore: 0,
                        rounds: [],
                        tutorialSeen: false,
                    ...
                    if (!p.rounds.includes(id)) {
                        if (attempt) {
                            ...
                            const stars = levelStars(level, attempt.values, score), levels = (_p = p).levels ?? (_p.levels = {}), old = levels[level.id];
                            levels[level.id] = {
                                completed: !!old?.completed || stars > 0,
                                stars: Math.max(old?.stars ?? 0, stars),
                                bestScore: Math.max(old?.bestScore ?? 0, score),
                                bestChain: Math.max(old?.bestChain ?? 0, attempt.bestChain),
            ...
            redeemPrize(day, slot) {
                return this.commit((data)=>{
                    ...
                    const shelf = data.prizes?.day === day ? data.prizes : data.prizes = {
                        day,
                        sold: []
                    };
            ...
            protect(id, kind, enabled) {
                this.commit((data)=>{
                    ...
                    const p = (_ref = (_data = data).protections ?? (_data.protections = {}))[_id = id] ?? (_ref[_id] = {
                        favorite: false,
                        locked: false
                    });
                    p[kind] = enabled;
            ...
            purchaseStock(site) {
                return this.commit((data)=>{
                    ...
                    const box = {
                        id: this.makeId(),
                        dumplingId: rollSeries(slot.series, store, this.random).id,
                        series: slot.series
                    };
                    data.balance -= price;
                    slot.remaining--;
            ...
            purchase() {
                return this.commit((data)=>{
                    ...
                    const box = {
                        id: this.makeId(),
                        dumplingId: rollDumpling(this.random).id
                    };
            ...
            constructor(repository, random, makeId = saveId){
                ...
                _define_property(this, "random", void 0);
                _define_property(this, "makeId", void 0);
                _define_property(this, "data", fresh());
                _define_property(this, "problem", "");
...
function createBlindBox(app, parent, compact = false) {
    const root = new Entity("Bamboo surprise steamer", app);
    parent.addChild(root);
    const model = steamerModel(app, compact);
    root.addChild(model);
    const lid = compact ? new Entity("Static shelf lid", app) : model.findByName("LidHinge");
...
function createDumpling(app, parent, data) {
    const root = new Entity(data.name, app);
    parent.addChild(root);
...
function createStore(app, definition2) {
    const root = new Entity(definition2.name, app);
    app.root.addChild(root);
    ...
    const floor = material(definition2.name + " floor", [
        "#c3d1b5",
        "#efd7e2",
        "#b9bfd4"
    ][definition2.layout]);
    surfaces.apply(floor, [
        ...
        "checker",
        "rug"
    ][definition2.layout], definition2.layout === 2 ? 5 : 8);
    ...
    shape("Shop garden surroundings", "box", [
        0,
        -0.22,
        0
    ], [
        ...
        0.18,
        40
    ], material("Shop grass", "#a6bc89"), false);
    for(let x = -3; x <= 3; x += 2)art.add("building", "floor", [
        x,
        -0.08,
        depth + 1
    ], 2, 0, "width");
    art.add("nature", "tree_oak", [
        -width - 1.8,
        -0.07,
        depth + 2.3
    ], 3.5);
    ...
    const exitAnchor = new Vec3(0, 0, depth - 0.65);
    shape("Welcome mat", "box", [
        0,
        0.012,
        exitAnchor.z
    ], [
    ...
    art.add("building", "wall-window-wide-round", [
        -width + 0.06,
        0.02,
        -1.9
    ], 2.55, 0, "height");
    art.add("building", "wall-doorway-round", [
        width - 1,
        0.01,
        -depth + 0.05
    ], 2.65, 90);
    art.add("building", "door-rotate-round-a", [
        width - 1,
        0.02,
        -depth + 0.1
    ], 2.1, 90);
    ...
    const sites = [
        ...
        },
        {
            ...
            box: [
                0.65,
                definition2.layout === 2 ? 0.8 : 0.53,
                0.45 - offset
            ],
            kind: definition2.layout === 2 ? "tableRound" : "tableCoffee"
        }
    ];
    const boxes = [], glows = [];
    for (const [i, site] of sites.entries()){
        ...
        art.add(market ? "market" : "furniture", site.kind, site.fixture, sizes[i], i === 1 ? 90 : i === 4 ? -90 : 0, i === 5 ? "width" : "height", {
            carpet: accent,
            wood: accent,
            woodDark: secondary,
            metal: ivory
        ...
        const half = i === 0 ? [
            ...
            0.76,
            definition2.layout === 2 ? 0.88 : 0.46
        ];
        ...
        const glow = shape("Nearby display aura", "cylinder", [
            site.point[0],
            0.027,
            site.point[2]
        ], [
            ...
            0.02,
            0.85
        ], material("Soft golden aura", "#ffe8ad"), false);
        glow.enabled = false;
        glows.push(glow);
    }
    art.add("market", "cash-register", [
        2.3,
        1.03,
        depth - 2.65
    ], 0.3, 90);
    art.add("market", "shopping-cart", [
        -width + 0.6,
        0.02,
        1.35
    ], 0.85, 180);
    obstacles.push(new BoundingBox(new Vec3(-width + 0.6, 0, 1.35), new Vec3(0.38, 1, 0.55)));
    art.add("market", "shelf-bags", [
        -width + 0.48,
        0.02,
        -depth + 2.2
    ], 1.1, 90);
    obstacles.push(new BoundingBox(new Vec3(-width + 0.48, 0, -depth + 2.2), new Vec3(0.32, 1, 0.65)));
    art.add("furniture", "bear", [
        -1.65,
        1.43,
        -depth + 0.62
    ], 0.3);
    art.add("furniture", "plantSmall1", [
        2.56,
        1.03,
        depth - 2.1
    ], 0.27);
    art.add("furniture", "pottedPlant", [
        width - 0.45,
        0.02,
        -0.2
    ], 1.05);
    obstacles.push(new BoundingBox(new Vec3(width - 0.45, 0, -0.2), new Vec3(0.33, 1, 0.33)));
    art.add("furniture", "lampRoundFloor", [
        -0.2,
        0.02,
        -depth + 0.45
    ], 1.9);
    if (definition2.layout === 1) {
        ...
        art.add("furniture", "benchCushion", [
            0.8,
            0.02,
            -depth + 1
        ], 0.75, 0, "height", {
        ...
        obstacles.push(new BoundingBox(new Vec3(0.8, 0, -depth + 1), new Vec3(0.65, 1, 0.42)));
        art.add("furniture", "bookcaseOpen", [
            -3.3,
            0.02,
            -4.3
        ], 1.8, 90);
        obstacles.push(new BoundingBox(new Vec3(-3.3, 0, -4.3), new Vec3(0.3, 1, 0.5)));
        art.add("furniture", "bear", [
            -3.3,
            0.68,
            -4.2
        ...
        obstacles.push(new BoundingBox(new Vec3(3.3, 0, 1.9), new Vec3(0.35, 1, 0.6)));
    }
    if (definition2.layout === 2) art.add("furniture", "plantSmall2", [
        0.65,
        0.8,
        0.08 - offset
    ], 0.3);
                ...
                if (slot) {
                    for (const render of box.findComponents("render"))for (const mesh of render.meshInstances){
                        ...
                        if (mesh.material.name.includes("Box blush")) {
                            ...
                            m.diffuse = new Color().fromString(seriesById(slot.series).color);
                            m.update();
                        }
                    }
                }
    ...
    return {
        ...
        sites: sites.map((s, i)=>({
                id: i,
                name: STOCK_SITES[i],
                anchor: new Vec3(...s.point),
                marker: new Vec3(s.box[0], s.box[1] + 0.6, s.box[2])
            })),
        boxes,
        glows,
        exitAnchor,
...
function squishyOpeningPose(seconds, intensity = 1) {
    if (seconds >= SQUISHY_REVEAL_SECONDS) return {
        lid: -112,
        height: 0.34,
        scale: [
            1,
    ...
    return {
        ...
        wiggle: seconds < 0.55 ? Math.sin(seconds * 31) * Math.sin(seconds / 0.55 * Math.PI) * 2 : 0
    };
}
        ...
        SquishyRevealVfx = class {
            ...
            ring(radius, width, alpha) {
                ...
                for(let i = 0; i <= 64; i++){
                    ...
                    for (const r of [
                        ...
                        radius,
                        radius + width
                    ])this.vertex(Math.cos(a) * r, Math.sin(a) * r, r === radius ? alpha : 0, 0.35);
                }
            ...
            snapshot() {
                return {
                    ...
                    drawCalls: this.root.enabled ? 1 : 0,
                    triangles: this.triangles / 3,
                    burstStars: this.style.sparkles,
                    maxIdleGlints: this.style.rays ? 4 : this.style.sparkles > 10 ? 3 : this.style.sparkles ? 2 : 0
                };
            }
            destroy() {
                this.root.destroy();
            ...
            constructor(app, parent){
                _define_property(this, "root", void 0);
                _define_property(this, "mesh", void 0);
                _define_property(this, "material", new StandardMaterial());
                _define_property(this, "positions", new Float32Array(1024 * 3));
                _define_property(this, "colors", new Float32Array(1024 * 4));
                _define_property(this, "indices", new Uint16Array(4096));
                _define_property(this, "vertices", 0);
                _define_property(this, "triangles", 0);
                _define_property(this, "previous", -100);
                _define_property(this, "style", SQUISHY_PRESENTATION.Common);
                _define_property(this, "color", new Color());
                this.root = new Entity("Rarity halo and celebration", app);
    ...
    "src/game/SquishyRevealAudio.ts" () {
        SquishyRevealAudio = class {
            ...
            constructor(){
                ...
                _define_property(this, "voices", /* @__PURE__ */ new Set());
            }
        };
    }
});
    ...
    "src/game/OpeningSequence.ts" () {
        ...
        OpeningSequence = class {
            ...
            frame(camera, width, height) {
                ...
                camera.lookAt(new Vec3(0, compact ? 0.5 : 0.85, 0));
            }
            show(receipt) {
            ...
            light(burst) {
                const s = SQUISHY_PRESENTATION[this.rarity], color = new Color().fromString(s.color), amount = this.phase === "closed" ? 0 : s.rim;
                this.rim.light.color = new Color(1 + (color.r - 1) * amount, 1 + (color.g - 1) * amount, 1 + (color.b - 1) * amount);
            ...
            snapshot() {
                return {
                    ...
                    rarity: this.rarity,
                    vfx: this.vfx.snapshot(),
                    lidAngle: this.box.lid.getLocalEulerAngles().x,
                    modelPosition: this.model?.getLocalPosition().toArray(),
                    modelScale: this.model?.getLocalScale().toArray()
                };
            }
            hide() {
                this.restoreCamera();
            ...
            constructor(app){
                _define_property(this, "app", void 0);
                _define_property(this, "root", void 0);
                _define_property(this, "box", void 0);
                _define_property(this, "model", null);
                _define_property(this, "elapsed", 0);
                _define_property(this, "last", 0);
                _define_property(this, "idle", 0);
                _define_property(this, "squish", -10);
                _define_property(this, "squishButton", document.createElement("button"));
                _define_property(this, "reduced", window.matchMedia("(prefers-reduced-motion: reduce)"));
                _define_property(this, "phase", "closed");
                _define_property(this, "receipt", null);
                _define_property(this, "audio", new SquishyRevealAudio());
                _define_property(this, "vfx", void 0);
                _define_property(this, "rim", void 0);
                _define_property(this, "rarity", "Common");
                _define_property(this, "camera", null);
                _define_property(this, "cameraHeight", 1.5);
                _define_property(this, "lidSounded", false);
                _define_property(this, "sounded", false);
                _define_property(this, "panel", document.querySelector("#reveal-copy"));
                this.app = app;
                this.root = new Entity("Home surprise presentation", app);
                ...
                ivory.gloss = 0.35;
                ivory.update();
                stage("Little presentation pedestal", "cylinder", [
                    0,
                    -0.045,
                    0
                ], [
                ...
                paper.emissive = new Color().fromString("#f6f0ec");
                paper.useTonemap = false;
                paper.update();
                const backdrop = new Entity("Reward backdrop", app);
                ...
                for (const [name, power, pitch, yaw] of [
                    [
                        "Reward key",
                        0.88,
                        38,
                    ...
                    const light = new Entity(name, app);
                    light.addComponent("light", {
                        type: "directional",
                        color: new Color(1, 0.98, 0.96),
                        intensity: power,
                        mask: 16,
                        castShadows: name === "Reward key",
...
var init_HuntUI = __esm({
    "src/ui/HuntUI.ts" () {
        ...
        HuntUI = class {
            ...
            constructor(choose){
                _define_property(this, "dialog", document.createElement("dialog"));
                _define_property(this, "time", document.createElement("p"));
                _define_property(this, "panel", document.createElement("section"));
                _define_property(this, "travel", document.createElement("section"));
                _define_property(this, "cards", /* @__PURE__ */ new Map());
                _define_property(this, "balance", document.createElement("p"));
                _define_property(this, "notice", document.createElement("p"));
...
function dailyWish(day, trader) {
    ...
    return pool[(day.day + [
        ...
        "series",
        "cute"
    ].indexOf(trader)) % pool.length] ?? DUMPLINGS[0];
}
function suggestTrade(day, trader, collection, protections = {}) {
    ...
    const search = (offer, start)=>{
        if (willing(day, trader, offer)) {
            const score = offer.reduce((s, id)=>s + ({
                    Common: 1,
                    Rare: 4,
                    Epic: 12,
                    Legendary: 30
                })[DUMPLINGS.find((d)=>d.id === id).rarity], 0) * 10 + offer.length;
            if (score < cost) {
                best = [
                    ...offer
                ];
        ...
        for(let i = start; i < ids.length; i++){
            const id = ids[i];
            if (offer.filter((x)=>x === id).length < (collection[id] ?? 0) - 1) search([
                ...offer,
                id
            ], i);
        }
      ...
      <div class="trade-footer"><p id="trade-readiness"></p><div class="trade-controls"><button id="trade-cancel" aria-label="Walk away">\u2715<small>Walk away</small></button><button id="trade-add" aria-label="Ask them to add">+<small>Add more?</small></button><button id="trade-accept" aria-label="Accept trade">\u2713<small>Trade</small></button></div></div>`;
                ...
                q("#trade-accept").onclick = ()=>{
                    this.attempt(()=>{
                        const last = [
                            ...new Set(this.give)
                        ].filter((id)=>this.save.data.collection[id] === this.give.filter((x)=>x === id).length);
            ...
            constructor(save, changed, exit){
                _define_property(this, "save", void 0);
                _define_property(this, "changed", void 0);
                _define_property(this, "exit", void 0);
                _define_property(this, "dialog", document.createElement("dialog"));
                _define_property(this, "leave", document.createElement("button"));
                _define_property(this, "trader", "rarity");
                _define_property(this, "give", []);
                _define_property(this, "includeLast", false);
                _define_property(this, "notice", "");
                _define_property(this, "day", 1);
                _define_property(this, "revision", 0);
                this.save = save;
...
function classroomSign(app, parent, name, text, position, width = 1, height = 0.26, color = "#5c496e") {
    ...
    material2.cull = CULLFACE_NONE;
    material2.update();
    const sign = new Entity(name, app);
    parent.addChild(sign);
    sign.addComponent("render", {
        type: "plane",
        material: material2,
        castShadows: false
    });
...
var init_Classmates = __esm({
    "src/game/Classmates.ts" () {
        ...
        Classmates = class {
            ...
            attachLayout(group) {
                const nodes = [
                    ...this.actors.map((a)=>a.model),
                    ...TRADERS.map((t)=>group.root.findByName(t.name + " nameplate"))
                ];
            ...
            update(now, focus) {
                ...
                for (const a of this.actors){
                    ...
                    a.arm.setLocalRotation(new Quat().mul2(a.armRest, new Quat().setFromEulerAngles(0, 0, wave * 45 + Math.sin(greeting * 14) * wave * 12)));
                    a.head.setLocalRotation(new Quat().mul2(a.headRest, new Quat().setFromEulerAngles(Math.sin(this.time * 1.4) * 2, a.near ? 0 : Math.sin(this.time * 0.5) * 5, 0)));
                }
            }
            snapshot() {
                return {
            ...
            constructor(app, parent){
                ...
                _define_property(this, "time", 0);
                _define_property(this, "last", 0);
                _define_property(this, "actors", []);
                _define_property(this, "signs", []);
                this.app = app;
                const names = [
                    "character-male-a",
                    "character-female-b",
                ...
                this.ready = Promise.all(TRADERS.map(async (t, i)=>{
                    ...
                    try {
                        ...
                        const asset = new Asset(t.name + " classmate", "container", {
                            url: assetUrl(path)
                        }, {}, containerOptions(path));
                        await new Promise((resolve, reject)=>{
                            asset.once("load", resolve);
                        ...
                        this.actors.push({
                            ...
                            head,
                            arm,
                            headRest: head.getLocalRotation().clone(),
                            armRest: arm.getLocalRotation().clone(),
                            id: t.id,
                            hello: -10,
                            near: false
                        });
...
function createRecess(app) {
    const root = new Entity("Classroom trading club", app);
    app.root.addChild(root);
    ...
    art.add("school", "blackboard", [
        0.1,
        1.05,
        -4.95
    ], 3.2, 0, "width");
    art.add("school", "bulletin-board", [
        -3.85,
        1.25,
        -2.8
    ], 1.5, 90, "width");
    ...
    art.add("furniture", "desk", [
        1.5,
        0.02,
        -3.9
    ], 0.88, 180, "height", schoolColors, false, 0, "paint");
    art.add("furniture", "chairDesk", [
        1.5,
        0,
        -4.65
    ], 0.95, 0, "height", schoolColors, false, 0, "paint");
    art.add("furniture", "books", [
        1.5,
        0.91,
        -3.9
    ], 0.22);
    art.add("furniture", "bookcaseOpen", [
        ...
        0.02,
        -4.2
    ], 1.7, 90, "height", schoolColors, false, 0, "paint");
    art.add("furniture", "books", [
        -3.25,
        0.8,
        -4.2
    ], 0.25);
    art.add("furniture", "coatRackStanding", [
        -3.3,
        0,
        2.6
    ], 1.5, 0, "height", schoolColors, false, 0, "paint");
    art.add("furniture", "trashcan", [
        3.3,
        0,
        -4.6
    ], 0.55, 0, "height", {
    ...
    const offers = new Entity("Squishies on the table", app);
    ...
    const seats = TRADERS.map((trader, i)=>{
        ...
        art.add("furniture", "desk", [
            ...
            0,
            0
        ], 0.78, 0, "height", schoolColors, false, 0, "paint");
        art.add("furniture", "chairDesk", [
            x,
            0,
            -0.75
        ...
        const glow = shape("Trading spot", "cylinder", [
            x,
            0.012,
            1.5
        ], [
            ...
            0.025,
            0.85
        ], material(trader.name + " cue", trader.color), false);
        return {
            id: trader.id,
            anchor: new Vec3(x, 0, 1.5),
            glow
        };
    });
    ...
    classroomSign(app, root, "Trading club board", "TRADING CLUB\nBring extras. Make friends.", [
        0.2,
        2.25,
        -4.91
    ], 2.75, 0.7, "#496d61");
    const clock = shape("Classroom clock", "cylinder", [
        2.8,
        2.5,
        -5.02
    ], [
    ...
    art.add("furniture", "benchCushion", [
        ...
        0,
        3.6
    ], 0.7, 90, "height", schoolColors, false, 0, "paint");
    const room = {
        root,
        halfWidth: 3.6,
        halfDepth: 4.7,
        ...
        obstacles: [
            new BoundingBox(new Vec3(0, 0, -0.3), new Vec3(2.6, 1, 1.25)),
            new BoundingBox(new Vec3(-3, 0, 3.6), new Vec3(0.45, 1, 0.8))
        ],
        ready: Promise.all([
            art.finish(),
            classmates.ready
        ...
        artStats: ()=>({
                ...art.snapshot(),
                classmates: classmates.snapshot()
            })
    };
    root.enabled = false;
    const sync = (day)=>{
    ...
    const bindLayout = (group)=>{
        ...
        for (const seat of seats){
            ...
            else group.getWorldTransform().transformPoint(seat.anchor.clone().add(new Vec3(0, 0, 0.3)), seat.anchor);
        }
    };
    return {
        ...room,
        ...
        sync,
        bindLayout,
        update: (dt, focus)=>classmates.update(dt, focus)
    };
}
    ...
    "src/ui/PopAudio.ts" () {
        ...
        PopAudio = class {
            async unlock() {
                ...
                this.loading ?? (this.loading = Promise.all([
                    "click_001",
                    "drop_001",
                    "drop_002",
                    "drop_003",
            ...
            pop(chain) {
                ...
                if (chain >= 8) {
                    ...
                    [
                        ...
                        1319,
                        1568
                    ].forEach((n, i)=>this.note(n, i * 0.075, 0.3, 0.15));
                    [
                        262,
                        330,
                        392
                    ].forEach((n)=>this.note(n, 0.42, 0.55, 0.08, "triangle"));
                } else if (chain >= 5) {
                    [
                        523,
                        659,
                        784,
                        1047
                    ].forEach((n, i)=>this.note(n, i * 0.07, 0.26, 0.14));
                }
            }
            power(kind) {
                if (kind === "bomb") {
                    ...
                    [
                        ...
                        440,
                        880
                    ].forEach((n, i)=>this.note(n, i * 0.065, 0.35, 0.2, "triangle"));
                    ...
                    [
                        ...
                        988,
                        1047
                    ].forEach((n, i)=>this.note(n, i * 0.06, 0.3, 0.13));
                } else [
                    660,
                    880,
                    1100,
                    1320
                ].forEach((n, i)=>this.note(n, i * 0.05, 0.2, 0.1));
            }
            countdown(n) {
            ...
            celebrate() {
                ...
                [
                    ...
                    1047,
                    1319
                ].forEach((n, i)=>this.note(n, i * 0.1, 0.3, 0.13));
            }
            setMusic(enabled) {
            ...
            constructor(){
                ...
                _define_property(this, "master", void 0);
                _define_property(this, "music", void 0);
                _define_property(this, "buffers", /* @__PURE__ */ new Map());
                _define_property(this, "voices", /* @__PURE__ */ new Set());
                _define_property(this, "loading", void 0);
                _define_property(this, "musicWanted", false);
                _define_property(this, "suspended", false);
                _define_property(this, "muted", false);
                _define_property(this, "failures", []);
                _define_property(this, "played", 0);
            }
        };
    }
});
...
var init_PopArt = __esm({
    "src/ui/PopArt.ts" () {
        ...
        PopArt = class {
            load() {
                return this.ready ?? (this.ready = this.loadAll().catch((error)=>{
                    this.ready = void 0;
                    throw error;
                }));
            }
            ...
            constructor(){
                _define_property(this, "friends", /* @__PURE__ */ new Map());
                _define_property(this, "powers", /* @__PURE__ */ new Map());
                _define_property(this, "ready", void 0);
            }
        };
    }
...
var init_SquishyPopUI = __esm({
    "src/ui/SquishyPopUI.ts" () {
        ...
        SquishyPopUI = class {
            ...
            start() {
                this.board = this.level ? createLevelBoard(this.level, {
                    ...this.collection()
                }) : new PopBoard(boardPool(this.collection()), {
                    ...this.collection()
                });
            ...
            showCover() {
                ...
                cover.hidden = [
                    ...
                    "lesson",
                    "finale"
                ].includes(this.state) && !this.paused;
                if (this.paused) {
                    ...
                    cover.innerHTML = `<h3>Little chains.<br>Big squishes!</h3><div class="pop-demo">${Array.from({
                        length: 3
                    }, ()=>`<img src="${this.images.get(this.board.pool[0]).toDataURL()}" alt="">`).join("")}<b>\u261D</b></div><p>Let\u2019s try one together. No timer yet!</p><button data-start>Show me how</button><button class="pop-link" data-back>Back to game</button>`;
                    cover.querySelector("[data-start]").addEventListener("click", ()=>{
                        ...
                        [
                            ...
                            13,
                            14
                        ].forEach((i)=>this.board.pieces[i].kind = this.board.pool[0]);
            ...
            results() {
                ...
                const cover = this.q(".pop-cover");
                cover.hidden = false;
                cover.innerHTML = `<span class="pop-result-star">${record ? "\u2605" : "\u2726"}</span><h3>${record ? "A new personal best!" : "So squishy!"}</h3><div class="pop-result-numbers"><span>Score<b data-result-score>${this.board.score.toLocaleString()}</b></span><span>Best chain<b>${this.board.bestChain}</b></span></div><small class="pop-previous-best">Previous best ${this.previousBest.toLocaleString()}</small><div class="pop-ticket-flight" aria-hidden="true">${Array.from({
                    length: this.developerPractice || error ? 0 : roundTickets(this.board.score)
                }, (_, i)=>`<img style="--i:${i}" src="${this.art.powers.get("ticket").toDataURL()}" alt="">`).join("")}</div><strong class="pop-ticket-prize">${error ? "Tickets not saved yet" : `+${roundTickets(this.board.score)} Squishy Tickets`}</strong><p>${error || `${total} tickets saved \xB7 Pick a squishy from the prize shelf, starting at 1 ticket!`}</p><progress max="8" value="${Math.min(total, 8)}"></progress><button data-replay>${error ? "Retry saving" : "PLAY AGAIN"}</button><button data-back>BACK TO GAME</button>`;
            ...
            selectable(index) {
                ...
                if (this.state === "lesson") return !this.lessonDoneAt && [
                    12,
                    13,
                    14
                ].includes(index);
            ...
            down(e) {
                if (this.pointer !== null || ![
                    "playing",
                    "lesson"
                ].includes(this.state) || this.paused || this.developerHold || this.remaining <= 0 || e.button !== 0) return;
                e.preventDefault();
            ...
            move(e) {
                ...
                for(let i = 1; i <= steps; i++){
                    const index = this.hit({
                        x: old.x + (next.x - old.x) * i / steps,
                        y: old.y + (next.y - old.y) * i / steps
                    });
            ...
            pop(result) {
                ...
                this.moves = new Map(result.moves.map((m)=>[
                        m.piece.id,
                        {
                            from: m.from,
                            to: m.to
            ...
            draw(dt) {
                ...
                for(let i = 0; i < 36; i++){
                    ...
                    c.save();
                    c.translate(x, y);
                    if (matching && piece.kind !== matching && piece.power !== "rainbow" && !preview.removed.has(i) || this.state === "lesson" && !this.lessonDoneAt && ![
                        12,
                        13,
                        14
                    ].includes(i)) c.globalAlpha = 0.3;
                    ...
                    if (this.state === "lesson" && !this.lessonDoneAt && [
                        12,
                        13,
                        14
                    ].includes(i)) {
            ...
            snapshot() {
                ...
                return {
                    open: this.isOpen,
                    level: this.level?.id,
                    objectives: this.levelRun?.values,
                    completed: this.levelRun?.completed,
                    state: this.state,
                    remaining: this.remaining,
                    paused: this.paused,
                    practice: this.developerPractice,
                    timerFrozen: this.developerFreeze,
                    chain: [
                        ...this.board.chain
                    ],
                    ...
                    bestChain: this.board.bestChain,
                    valid: this.board.findChain(),
                    pool: this.board.pool,
                    particles: this.particles.filter((p)=>p.life > 0).length,
                    frenzy: this.frenzyWas,
                    frenzyMeter: this.board.frenzyMeter,
                    lastResult: this.lastResult ? {
                        chain: this.lastResult.chain,
                        created: this.lastResult.created,
                        activated: this.lastResult.activated,
                        shuffled: this.lastResult.shuffled
                    } : null,
                    audio: this.audio.snapshot(),
                    frameP95: sorted[Math.floor(sorted.length * 0.95)] ?? 0,
                    artFriends: this.images.size
                };
            }
            ...
            developerStatus() {
                return {
                    ready: this.prepared,
                    practice: this.developerPractice,
                    freeze: this.developerFreeze,
                    held: this.developerHold
                };
            }
            developerControl(command, value) {
                ...
                this.lockedUntil = 0;
                if (command === "chain") {
                    const n = Math.max(3, Math.min(12, value ?? 7)), path = [
                        12,
                        13,
                        14,
                        15,
                    ...
                    for(let i = 35; i > 0; i--){
                        ...
                        [this.board.pieces[i], this.board.pieces[j]] = [
                            this.board.pieces[j],
                            this.board.pieces[i]
                        ];
                    }
            ...
            constructor(collection, award, onOpen, firstPlay = ()=>true, record = ()=>({
                    bestScore: 0,
                    tickets: 0
                })){
                _define_property(this, "collection", void 0);
                _define_property(this, "award", void 0);
                _define_property(this, "onOpen", void 0);
                _define_property(this, "firstPlay", void 0);
                _define_property(this, "record", void 0);
                _define_property(this, "onPrizes", ()=>{});
                _define_property(this, "dialog", document.createElement("dialog"));
                _define_property(this, "launch", document.createElement("button"));
                _define_property(this, "canvas", void 0);
                _define_property(this, "ctx", void 0);
                _define_property(this, "board", new PopBoard(boardPool({})));
                _define_property(this, "art", new PopArt());
                _define_property(this, "images", this.art.friends);
                _define_property(this, "state", "levels");
                _define_property(this, "level", void 0);
                _define_property(this, "levelRun", void 0);
                _define_property(this, "goalMarkup", "");
                _define_property(this, "remaining", POP_RULES.seconds);
                _define_property(this, "countdown", 3.8);
                _define_property(this, "activeTime", 0);
                _define_property(this, "last", 0);
                _define_property(this, "frame", 0);
                _define_property(this, "pointer", null);
                _define_property(this, "previous", null);
                _define_property(this, "lockedUntil", 0);
                _define_property(this, "moves", /* @__PURE__ */ new Map());
                _define_property(this, "particles", Array.from({
                    length: 90
                }, ()=>({
                        x: 0,
                        y: 0,
                ...
                _define_property(this, "burst", 0);
                _define_property(this, "feedback", "");
                _define_property(this, "feedbackUntil", 0);
                _define_property(this, "paused", false);
                _define_property(this, "abort", new AbortController());
                _define_property(this, "observer", void 0);
                _define_property(this, "size", 360);
                _define_property(this, "receipt", "");
                _define_property(this, "saved", false);
                _define_property(this, "audio", new PopAudio());
                _define_property(this, "lastSecond", 60);
                _define_property(this, "lastCount", 4);
                _define_property(this, "frenzyWas", false);
                _define_property(this, "gone", []);
                _define_property(this, "lastResult", void 0);
                _define_property(this, "frameSamples", []);
                _define_property(this, "reduced", matchMedia("(prefers-reduced-motion: reduce)").matches);
                _define_property(this, "developerHold", false);
                _define_property(this, "developerPractice", false);
                _define_property(this, "developerFreeze", false);
                _define_property(this, "prepared", false);
                _define_property(this, "prepareVersion", 0);
                _define_property(this, "finaleAt", 0);
                _define_property(this, "finaleDone", false);
                _define_property(this, "resultsAt", 0);
                _define_property(this, "previousBest", 0);
                _define_property(this, "ticketTotal", 0);
                _define_property(this, "lessonDoneAt", 0);
                _define_property(this, "chainCue", "");
                _define_property(this, "tick", (now)=>{
                ...
                this.dialog.querySelector(".pop-pause").addEventListener("click", ()=>this.togglePause(), {
                    signal
                });
                ...
                for (const name of [
                    "pointercancel",
                    "lostpointercapture"
                ])this.canvas.addEventListener(name, ()=>this.cancel(), {
                    signal
                });
    ...
    "src/ui/TicketShop.ts" () {
        ...
        TicketShop = class {
            ...
            constructor(save, day, changed){
                _define_property(this, "save", void 0);
                _define_property(this, "day", void 0);
                _define_property(this, "changed", void 0);
                _define_property(this, "dialog", document.createElement("dialog"));
                _define_property(this, "art", new PopArt());
                _define_property(this, "notice", "");
    ...
    "src/game/GameLoop.ts" () {
        ...
        GameLoop = class {
            ...
            renderCollection() {
                ...
                for (const series of SERIES){
                    ...
                    for (const data of DUMPLINGS.filter((d)=>series.items.includes(d.id))){
                        ...
                        grid.append(card);
                        if (count) for (const [kind, icon, label] of [
                            [
                                "favorite",
                                "\u2665",
                                "Favorite"
            ...
            developerSummary() {
                return {
                    scene: this.mode,
                    store: this.mode === "store" ? this.store.definition.name : null,
                    day: this.props.daily.clock.state.day,
                    time: this.props.daily.clock.label,
                    phase: this.props.daily.clock.state.phase,
                    clockFrozen: this.developerClockFrozen,
                    balance: this.save.data.balance,
                    tickets: this.save.data.pop?.tickets ?? 0,
                    discovered: DUMPLINGS.filter((d)=>this.save.data.collection[d.id] > 0).length,
                    boxes: this.save.data.boxes.length,
                    cleanup: this.cleanup.mode,
                    completed: this.cleanup.mission.completed.size,
                    tasks: this.cleanup.mission.tasks.length,
                    pop: this.popUI.snapshot(),
                    popDev: this.popUI.developerStatus()
                };
            }
                ...
                switch(command){
                    ...
                    case "phase":
                        if (![
                            ...
                            "afternoon",
                            "night"
                        ].includes(value)) throw Error("Choose a time of day.");
                        phase(value);
                        break;
                    case "next-day":
                        phase("morning", true);
            ...
            snapshot() {
                return {
                    ...
                    reveal: this.save.data.reveal ? {
                        ...this.save.data.reveal
                    } : null,
                    focus: this.focus,
                    ...
                    recess: this.mode === "recess" ? {
                        seats: this.recess.seats.map((s)=>({
                                id: s.id,
                                position: s.anchor.toArray()
                            })),
                        art: this.recess.artStats?.()
                    } : null,
                    hunt: this.save.data.hunt,
                    store: this.mode === "store" ? {
                        id: this.activeStoreId,
                        sites: this.store.sites.map((s)=>({
                                id: s.id,
                                position: s.anchor.toArray()
                            })),
                        exit: this.store.exitAnchor.toArray(),
                        walkable: this.store.walkable,
                        obstacles: this.store.obstacles.map((b)=>({
                                center: b.center.toArray(),
                                halfExtents: b.halfExtents.toArray()
                            })),
                        art: this.store.artStats?.()
                    } : null
                };
            }
            destroy() {
            ...
            constructor(app, camera, character, room, props, cleanup, controller, joystick){
                ...
                _define_property(this, "room", void 0);
                _define_property(this, "props", void 0);
                _define_property(this, "cleanup", void 0);
                _define_property(this, "controller", void 0);
                _define_property(this, "joystick", void 0);
                _define_property(this, "tornado", void 0);
                _define_property(this, "save", new ProgressStore(new LocalSaveRepository()));
                _define_property(this, "stores", void 0);
                _define_property(this, "activeStoreId", "corner");
                _define_property(this, "huntUI", void 0);
                _define_property(this, "tradingUI", void 0);
                _define_property(this, "popUI", void 0);
                _define_property(this, "ticketShop", void 0);
                _define_property(this, "nextStore", document.createElement("button"));
                _define_property(this, "recess", void 0);
                _define_property(this, "recessFromSchool", false);
                _define_property(this, "travelUntil", 0);
                _define_property(this, "inspecting", null);
                _define_property(this, "findCopy", "");
                _define_property(this, "lastHuntRefresh", 0);
                _define_property(this, "opening", void 0);
                _define_property(this, "mode", "cleanup");
                _define_property(this, "focus", "");
                _define_property(this, "action", void 0);
                _define_property(this, "abort", new AbortController());
                _define_property(this, "messageUntil", 0);
                _define_property(this, "pendingCredit", null);
                _define_property(this, "baseZoom", 9);
                _define_property(this, "screen", new Vec3());
                _define_property(this, "markerPoint", new Vec3());
                _define_property(this, "developerPaused", false);
                _define_property(this, "developerClockFrozen", false);
                _define_property(this, "press", ()=>this.attempt(()=>{
                        ...
                        if (this.mode === "store") {
                            ...
                            if (this.focus.startsWith("hunt-site-")) {
                                ...
                                if (!slot.discovered) {
                                    this.inspecting = {
                                        site,
                                        until: performance.now() + HUNT_RULES.inspectMilliseconds
                                    };
                                    this.joystick.reset();
                                    this.controller.reset();
                                } else {
                ...
                }, ()=>!this.save.data.pop?.tutorialSeen, ()=>({
                        bestScore: this.save.data.pop?.bestScore ?? 0,
                        tickets: this.save.data.pop?.tickets ?? 0,
                        levels: this.save.data.pop?.levels ?? {}
                    }));
                ...
                tradeButton.addEventListener("click", ()=>this.attempt(()=>this.enterRecess(false)), {
                    signal: this.abort.signal
                });
                for (const mode of [
                    "day",
                ...
                el("#collection-dialog").addEventListener("cancel", (e)=>e.preventDefault(), {
                    signal: this.abort.signal
                });
...
var init_HouseNavigation = __esm({
    "src/ui/HouseNavigation.ts" () {
        ...
        HouseNavigation = class {
            ...
            constructor(){
                _define_property(this, "current", "bedroom");
                _define_property(this, "root", document.querySelector("#house-doors"));
                _define_property(this, "point", new Vec3());
                _define_property(this, "screen", new Vec3());
                _define_property(this, "labels", HOUSE_DOORS.map((door)=>{
        ...
        Lilah = class {
            ...
            updateTornado(dt, arianna, velocity) {
                ...
                if (this.route.length) {
                    ...
                    if (separation < 0.43 && separation <= previousSeparation) {
                        job.blocked += dt;
                        if (job.blocked > 0.45) {
                            const obstacle = new BoundingBox(new Vec3(arianna.x, 0, arianna.z), new Vec3(0.27, 2, 0.27)), planner = new HousePath({
                                ...this.house,
                                obstacles: [
                                    ...this.house.obstacles,
                                    obstacle
                                ]
                            }, 0.18), start = new Vec3(p.x, 0, p.z);
                            let path = planner.route(start, job.point);
                            if (!path.length) {
                                for(let i = 0; i < 16; i++){
                                    const angle = i * Math.PI / 8, escape = new Vec3(p.x + Math.sin(angle) * 0.65, 0, p.z + Math.cos(angle) * 0.65);
                    ...
                    if (this.planner.free(q.x, q.z)) {
                        ...
                        this.route = this.planner.route(new Vec3(p.x, 0, p.z), job.point);
                    }
                } else {
                    if (!job.wait) {
                        this.say(job.icon + "  \u2026");
                    ...
                    }, new Vec3(job.point.x, 0.1, job.point.z + 0.3));
                }
            }
            async load() {
                const config = await (await fetch(assetUrl(`${"/"}assets/characters/arianna/character.json`))).json();
                this.height = config.height * 0.625;
                const asset = new Asset("Lilah Meshy review", "container", {
                    url: assetUrl(`${"/"}assets/characters/lilah/lilah.glb`)
                });
                await new Promise((resolve, reject)=>{
                    asset.once("load", resolve);
                    asset.once("error", reject);
                ...
                const model = resource.instantiateRenderEntity({
                    castShadows: true
                }), tracks2 = resource.animations.map((a)=>a.resource);
                ...
                for (const required of [
                    ...
                    "PutDown",
                    "Celebrate"
                ])if (!tracks2.some((t)=>t.name === required)) throw new Error("Missing Lilah clip " + required);
                const alignment = new Entity("Lilah ground alignment", this.app);
            ...
            decide(arianna) {
                ...
                if (clock.state.phase === "night") {
                    ...
                    this.toy.enabled = false;
                    this.animator.setCarrying(false);
                    this.go(propPoint("crib", new Vec3(8.55, 0, -1.3)), "bedtime");
                ...
                if (Math.random() < 0.55) {
                    ...
                    ])if (this.go(new Vec3(arianna.x + x, 0, arianna.z + z), "follow")) break;
                    ...
                    this.go(new Vec3(p[0], 0, p[1]), "explore");
            ...
            update(dt, elapsed, visible, canMischief, arianna, camera) {
                ...
                if (this.day !== this.daily.clock.state.day) {
                    ...
                    if (this.state === "sleeping" || this.bedStart) this.root.setPosition(propPoint("crib", new Vec3(8.55, 0.09, -1.3)));
                    this.bedStart = null;
                    this.state = "watching";
                }
                if ((this.state === "sleeping" || this.bedStart) && this.daily.clock.state.phase !== "night") {
                    this.root.setPosition(propPoint("crib", new Vec3(8.55, 0.09, -1.3)));
                    this.state = "watching";
                    this.bedStart = null;
                ...
                const screen = camera.camera.worldToScreen(new Vec3(p.x, p.y + this.height + 0.12, p.z));
                ...
                if (this.destination === "mess") {
                    this.animator.playAction("PutDown", 0.8, ()=>{
                        ...
                        this.say(made ? [
                            ...
                            "Uh-oh. All wet!",
                            "Crumbs are confetti!"
                        ][this.daily.lilahMesses.count - 1] : "I did it!");
                    ...
                    this.bedStart = {
                        position: this.root.getPosition().clone(),
                        yaw: this.visual.getLocalEulerAngles().y,
                        elapsed: 0
                    };
            ...
            snapshot() {
                return {
                    ...
                    job: this.job ? {
                        point: this.job.point.toArray(),
                        icon: this.job.icon,
                        wait: this.job.wait
                    } : null,
                    speech: this.speech,
                    carrying: this.carrying,
                    animation: this.animator.snapshot(),
                    visited: [
                        ...this.visited
                    ],
                    routeLength: this.route.length,
                    time: this.time,
                    nextMess: this.nextMess
                };
            }
            geometry() {
            ...
            constructor(app, house, daily){
                _define_property(this, "app", void 0);
                _define_property(this, "house", void 0);
                _define_property(this, "daily", void 0);
                _define_property(this, "root", void 0);
                _define_property(this, "visual", void 0);
                _define_property(this, "animator", void 0);
                _define_property(this, "planner", void 0);
                _define_property(this, "toy", void 0);
                _define_property(this, "socket", void 0);
                _define_property(this, "grounding", null);
                _define_property(this, "label", document.createElement("div"));
                _define_property(this, "route", []);
                _define_property(this, "destination", "");
                _define_property(this, "nextDecision", 0);
                _define_property(this, "nextMess", 8);
                _define_property(this, "time", 0);
                _define_property(this, "day", 0);
                _define_property(this, "height", 0.86465625);
                _define_property(this, "loaded", false);
                _define_property(this, "carrying", false);
                _define_property(this, "state", "watching");
                _define_property(this, "bedStart", null);
                _define_property(this, "speech", "Hi, Ari!");
                _define_property(this, "speechUntil", 0);
                _define_property(this, "visited", /* @__PURE__ */ new Set());
                _define_property(this, "scripted", false);
                _define_property(this, "job", null);
                this.app = app;
                this.house = house;
                this.daily = daily;
                this.root = new Entity("Lilah \xB7 age 2", app);
                app.root.addChild(this.root);
                this.root.setPosition(1, 0.09, 0.7);
                this.visual = new Entity("Lilah visual", app);
                this.root.addChild(this.visual);
                const placeholder = new Entity("Lilah loading", app);
                ...
                this.socket = new Entity("Lilah toy grip", app);
                this.visual.addChild(this.socket);
                this.animator.bindCarrySocket(this.socket);
                this.toy = primitives(app, this.socket)("Favorite block", "box", [
                    0,
                    0,
                    0
                ], [
                    ...
                    0.13,
                    0.13
                ], material("Lilah favorite block", "#edb867"));
                this.toy.enabled = false;
...
var init_TornadoRules = __esm({
    "src/systems/TornadoRules.ts" () {
        ...
        TornadoScore = class {
            ...
            constructor(){
                _define_property(this, "score", 0);
                _define_property(this, "cleaned", 0);
                _define_property(this, "streak", 0);
                _define_property(this, "bestStreak", 0);
                _define_property(this, "lastClean", -Infinity);
            }
        };
    }
});
    ...
    "src/game/LilahTornado.ts" () {
        ...
        LilahTornado = class {
            ...
            begin() {
                ...
                ].map(([x, z])=>new Vec3(x, 0, z)).filter((p)=>this.planner.free(p.x, p.z) && this.planner.route(new Vec3(this.character.player.getPosition().x, 0, this.character.player.getPosition().z), p).length > 0);
                this.lilah.beginTornado();
                void this.audio.unlock();
                this.audio.pause(false);
                get("#game-canvas").focus({
                    preventScroll: true
                });
            }
            /** Called before movement so daily time and other action listeners cannot compete. */ beforeMovement(now) {
            ...
            update(dt) {
                ...
                if (!this.lilah.working && this.elapsed >= this.nextDrop && this.messes.length < 3) {
                    ...
                    const candidates = this.spots.map((p, i)=>({
                            p,
                            i,
                            d: Math.hypot(p.x - position.x, p.z - position.z)
                        })).filter((v)=>v.i !== this.lastSpot && v.d > 0.8 && v.d < 12 && !this.messes.some((m)=>m.point.distance(v.p) < 1));
            ...
            spawn(point, type, special) {
                ...
                const root = new Entity("Tornado " + (special === "none" ? TYPES[type].name : special), this.app);
                ...
                if (special === "dog") {
                    ...
                    const part = shape("Mess piece", type === 3 ? "sphere" : type === 1 ? "cylinder" : "box", [
                        x,
                        type === 3 ? 0.02 : 0.07,
                        z
                    ], type === 3 ? [
                        ...
                        0.13,
                        0.13
                    ], type === 3 ? this.juice : this.colors[i % 4], false);
                    if (type === 1) part.setLocalEulerAngles(90, 0, i * 38);
                }
                if (special === "basket") {
                    const basket = shape("Empty toy basket", "box", [
                        0.35,
                        0.14,
                        0
                    ], [
            ...
            focus() {
                const p = this.character.player.getPosition();
                return this.messes.filter((m)=>Math.hypot(p.x - m.point.x, p.z - m.point.z) < 1.15 && this.sight.line(new Vec3(p.x, 0, p.z), m.point)).sort((a, b)=>a.point.distance(p) - b.point.distance(p))[0];
            }
            clean() {
            ...
            startDog() {
                ...
                if (!point) return;
                const path = this.planner.route(new Vec3(p.x, 0, p.z), point);
                if (!path.length) {
                ...
                this.dog = {
                    home: p.clone(),
                    angles: pet.dog.getEulerAngles().clone(),
                    route: path,
                    point: point.clone(),
                    wait: 0
                };
            ...
            updateDog(dt) {
                ...
                if (dog.route.length) {
                    const p = root.getPosition(), next = dog.route[0], delta = new Vec3(next.x - p.x, 0, next.z - p.z), distance = delta.length();
                    if (distance < 0.04) {
                        dog.route.shift();
                        return;
                    }
                    ...
                    if (dog.wait > 1.2 && this.messes.length < 3) {
                        ...
                        dog.wait = -1;
                        dog.route = this.planner.route(dog.point, new Vec3(dog.home.x, 0, dog.home.z));
                    }
                }
            }
            paint() {
                ...
                for (const m of this.messes){
                    const p = this.camera.entity.camera.worldToScreen(new Vec3(m.point.x, 0.4, m.point.z)), x = Math.max(24, Math.min(bounds.width - 24, p.x)), y = Math.max(this.hud.offsetTop + this.hud.offsetHeight + 40, Math.min(bounds.height - 180, p.y));
            ...
            snapshot() {
                return {
                    ...
                    cleaned: this.score.cleaned,
                    streak: this.score.streak,
                    bestStreak: this.score.bestStreak,
                    stars: this.score.stars,
                    reward: this.score.reward,
                    paid: this.paid,
                    special: this.special,
                    specialStarted: this.specialStarted,
                    created: this.created,
                    cleaning: this.cleaning?.mess.id,
                    aligning: this.cleaning?.aligning,
                    spots: this.spots.map((p)=>p.toArray()),
                    messes: this.messes.map((m)=>({
                            id: m.id,
                            point: m.point.toArray(),
                            type: m.type,
                            special: m.special
                        })),
                    audio: this.audio.snapshot()
            ...
            constructor(app, house, props, cleanup, loop, character, controller, camera, lilah){
                ...
                _define_property(this, "props", void 0);
                _define_property(this, "cleanup", void 0);
                _define_property(this, "loop", void 0);
                _define_property(this, "character", void 0);
                _define_property(this, "controller", void 0);
                _define_property(this, "camera", void 0);
                _define_property(this, "lilah", void 0);
                _define_property(this, "phase", "idle");
                _define_property(this, "elapsed", 0);
                _define_property(this, "score", new TornadoScore());
                _define_property(this, "round", "");
                _define_property(this, "paid", false);
                _define_property(this, "special", "none");
                _define_property(this, "specialStarted", false);
                _define_property(this, "created", 0);
                _define_property(this, "nextDrop", 0);
                _define_property(this, "messes", []);
                _define_property(this, "cleaning", null);
                _define_property(this, "planner", void 0);
                _define_property(this, "sight", void 0);
                _define_property(this, "audio", new PopAudio());
                _define_property(this, "launch", document.createElement("button"));
                _define_property(this, "hud", document.createElement("section"));
                _define_property(this, "dialog", document.createElement("dialog"));
                _define_property(this, "overlay", document.createElement("div"));
                _define_property(this, "action", void 0);
                _define_property(this, "abort", new AbortController());
                _define_property(this, "colors", [
                    "#edabbe",
                    "#a5c9e1",
                    "#ecc76c",
                    "#b3c895"
                ].map((c, i)=>material("Tornado toy " + i, c)));
                _define_property(this, "juice", material("Tornado juice", "#eeb360"));
                _define_property(this, "cloth", void 0);
                _define_property(this, "spots", []);
                _define_property(this, "lastSpot", -1);
                _define_property(this, "notice", "Follow Lilah\u2019s little trail!");
                _define_property(this, "noticeUntil", 0);
                _define_property(this, "recentSpots", []);
                _define_property(this, "lastType", -1);
                _define_property(this, "dog", null);
                _define_property(this, "popups", []);
                this.app = app;
                this.props = props;
                this.cleanup = cleanup;
                ...
                this.cloth = primitives(app, props.root)("Tornado cleaning cloth", "box", [
                    0,
                    0,
                    0
                ], [
                    ...
                    0.014,
                    0.17
                ], material("Tornado cloth", "#f9efe1"), false);
                this.cloth.enabled = false;
                ...
                document.addEventListener("visibilitychange", ()=>this.audio.pause(document.hidden), {
                    signal
                });
            }
        };
...
var init_Marc = __esm({
    "src/game/Marc.ts" () {
        ...
        init_house();
        init_primitives();
        SEAT = new Vec3(4.5, 0, 7.35);
        YAW = -35;
        FORWARD = new Vec3(Math.sin(YAW * Math.PI / 180), 0, Math.cos(YAW * Math.PI / 180));
        ...
        REMARKS = [
            "Need a hand, sweetie?",
            "I came. I saw. I stepped on a block.",
            "Sweetie, is this a house or a tiny toy museum?",
            "My coffee has been reheated three times. A new record.",
            "The laundry and I are in a long-term relationship.",
            "Nice collecting, sweetie. I collect missing socks."
        ];
        Marc = class {
            async load() {
                const config = await (await fetch(assetUrl(`${"/"}assets/characters/arianna/character.json`))).json();
                this.height = config.height * 1.3;
                const asset = new Asset("Marc animation v2", "container", {
                    url: assetUrl(`${"/"}assets/characters/marc/marc.glb`)
                });
                await new Promise((resolve, reject)=>{
                    asset.once("load", resolve);
                    asset.once("error", reject);
                ...
                const model = resource.instantiateRenderEntity({
                    castShadows: true
                }), source = resource.animations.map((a)=>a.resource);
                const required = (name)=>{
                ...
                const tracks2 = [
                    ...source,
                    new AnimTrack("Walk", walk.duration, walk.inputs, walk.outputs, walk.curves)
                ];
                ...
                for (const curve of idle.curves){
                    ...
                    if (paths.some((p)=>p.entityPath.at(-1) === "Spine02" && p.propertyPath[0] === "localRotation")) {
                        ...
                        for(let i = 0; i < data.length; i += 4){
                            const q = new Quat(data[i], data[i + 1], data[i + 2], data[i + 3]).mul(new Quat().setFromEulerAngles(24, 0, 0));
                            data[i] = q.x;
                            data[i + 1] = q.y;
                            data[i + 2] = q.z;
                            data[i + 3] = q.w;
                ...
                tracks2.push(new AnimTrack("Cleaning", idle.duration, idle.inputs, outputs, idle.curves));
                const manifest = {
                    animations: tracks2.map((t)=>({
                            name: t.name,
                            duration_seconds: t.duration,
                ...
                const alignment = new Entity("Marc ground alignment", this.app);
                this.visual.addChild(alignment);
                alignment.addChild(model);
            ...
            approachMess() {
                ...
                for (const radius of [
                    ...
                    const a = i * Math.PI / 6, v = new Vec3(m.x + Math.cos(a) * radius, 0, m.z + Math.sin(a) * radius);
            ...
            update(dt, elapsed, visible, active, arianna, lilah, playerTarget, camera) {
                ...
                if (!this.root.enabled || document.hidden) return;
                if (!active) {
                    this.animator.update(0, new Vec3(), Math.max(elapsed, 1e-3));
                    return;
                }
                this.time += dt;
                this.velocity.set(0, 0, 0);
                ...
                if (this.state === "walking") {
                            ...
                            if (distance <= step + 1e-5) {
                                ...
                                if (!this.route.length) {
                                    if (this.purpose === "seat") {
                                        ...
                                        this.animator.faceTowards(new Vec3(mess.x, 0, mess.z));
                                        this.say([
                                            "These blocks are plotting against my feet.",
                                            "Ah, floor juice. My least favorite flavor.",
                                            "Crumbs: the glitter of snack time."
                                        ][Number(mess.id.at(-1))]);
                                    } else {
                                        this.state = "idle";
                    ...
                    const p2 = new Vec3().lerp(propPoint("marc-seat", down ? ENTRY : SEATED), propPoint("marc-seat", down ? SEATED : ENTRY), smooth2);
                    ...
                    if (this.purpose === "seat" || this.purpose === "mess") {
                        ...
                        if (!this.go(new Vec3(point[0], 0, point[1]), "wander")) this.until = this.time + 3;
                ...
                const screen = camera.camera.worldToScreen(new Vec3(p.x, p.y + this.height + 0.1, p.z)), viewport = document.querySelector("#game").getBoundingClientRect();
            ...
            snapshot() {
                return {
                    ...
                    state: this.state,
                    target: this.target,
                    route: this.route.map((p)=>p.toArray()),
                    speech: this.speech,
                    cleaned: this.cleaned,
                    sitCount: this.sitCount,
            ...
            constructor(app, house, daily){
                ...
                _define_property(this, "route", []);
                _define_property(this, "loaded", false);
                _define_property(this, "height", 1.798485);
                _define_property(this, "state", "idle");
                _define_property(this, "purpose", "seat");
                _define_property(this, "time", 0);
                _define_property(this, "until", 1);
                _define_property(this, "transitionStart", 0);
                _define_property(this, "day", 0);
                _define_property(this, "target", null);
                _define_property(this, "seen", /* @__PURE__ */ new Map());
                _define_property(this, "speech", "");
                _define_property(this, "speechUntil", 0);
                _define_property(this, "nextSpeech", 6);
                _define_property(this, "lineIndex", 0);
                _define_property(this, "patrolIndex", 0);
                _define_property(this, "cleaned", 0);
                _define_property(this, "standCount", 0);
                _define_property(this, "sitCount", 0);
                _define_property(this, "visited", /* @__PURE__ */ new Set());
                _define_property(this, "velocity", new Vec3());
                _define_property(this, "blockedFor", 0);
                _define_property(this, "nextScan", 0);
                this.app = app;
                this.house = house;
                this.daily = daily;
                this.root = new Entity("Marc", app);
                app.root.addChild(this.root);
                this.root.setPosition(3.5, 0.09, 6.2);
                this.visual = new Entity("Marc visual", app);
                this.root.addChild(this.visual);
                const placeholder = new Entity("Marc loading", app);
                this.visual.addChild(placeholder);
                this.animator = new CharacterAnimator(this.visual, placeholder);
                ...
                for(let i = 0; i < 3; i++){
                    ...
                    shape("Dad cleanup tool", "box", [
                        ...
                        0.07,
                        0.13
                    ], material("Dad tool " + i, colors[i]));
                    if (i === 2) shape("Brush handle", "capsule", [
                        0,
                        0.18,
                        0
                    ], [
...
var init_DeveloperSaves = __esm({
    "src/dev/DeveloperSaves.ts" () {
        KEYS = [
            "dumpling.editorMigration.progress.v1",
            "dumpling.editorMigration.daily.v1",
            "dumpling.editorMigration.lilah.v1"
        ];
        ...
        DeveloperSaves = class {
            ...
            reset() {
                this.ensure();
                this.write(Object.fromEntries(KEYS.map((k)=>[
                        k,
                        null
                    ])));
            }
            current() {
                return {
                    version: 1,
                    created: /* @__PURE__ */ new Date().toISOString(),
                    values: Object.fromEntries(KEYS.map((k)=>[
                            k,
            ...
            tool(action) {
                try {
                    if (action === "capture") {
                        ...
                        this.download("arianna-diagnostics", {
                            at: /* @__PURE__ */ new Date().toISOString(),
                            game: this.loop.developerSummary(),
                            render: this.metrics(),
                            recentActions: this.log
                        });
...
Playable chain: ${s.pop.valid.length} pieces`);
            ...
            constructor(loop, metrics){
                ...
                _define_property(this, "dialog", document.createElement("dialog"));
                _define_property(this, "saves", new DeveloperSaves());
                _define_property(this, "abort", new AbortController());
                _define_property(this, "timer", void 0);
                _define_property(this, "launchers", []);
                _define_property(this, "observers", []);
                _define_property(this, "log", []);
                this.loop = loop;
      ...
      <nav class="dev-tabs" role="tablist" aria-label="Developer tools">${[
                    "Jump in",
                    "World",
                    "Pop lab",
                    "Save & tools"
                ].map((name, i)=>`<button role="tab" id="dev-tab-${i}" aria-controls="dev-section-${i}" aria-selected="${i === 0}" tabindex="${i === 0 ? 0 : -1}" data-tab="${i}">${name}</button>`).join("")}</nav>
      <div class="dev-content">
        ...
        <section role="tabpanel" id="dev-section-2" aria-labelledby="dev-tab-2" hidden>
          ...
          <div class="dev-card"><h3>Repeatable test boards</h3><p>Chains begin at the left of row 3, go right, then back along row 4. Resume to drag them.</p><div class="dev-grid">${[
                    3,
                    5,
                    7,
                    10
                ].map((n)=>button(`${n}-chain${n === 5 ? " \u2192 Bomb" : n === 7 ? " \u2192 Rainbow" : n === 10 ? " \u2192 Mega" : ""}`, "pop:chain", String(n))).join("")}${button("Place Pop Bomb", "pop:bomb")}${button("Place Rainbow", "pop:rainbow")}${button("Place Mega", "pop:mega")}${button("Trigger Frenzy", "pop:frenzy")}${button("Shuffle board", "pop:shuffle")}${button("Test no-move recovery", "pop:deadlock")}</div></div>
      ...
      <footer class="dev-bottom"><span>\u2161 World paused while this panel is open</span><small>Changes affect this local save. A checkpoint is kept before your first change.</small><div data-status role="status">Ready when you are.</div></footer>
                ...
                this.dialog.querySelector(".dev-close").addEventListener("click", ()=>this.close(), {
                    signal
                });
                this.dialog.querySelector(".dev-resume").addEventListener("click", ()=>this.close(), {
                    signal
                });
                this.dialog.addEventListener("click", (e)=>{
                    ...
                    if (target.dataset.command) {
                        ...
                        if ([
                            "give-item",
                            "give-box"
                        ].includes(target.dataset.command)) value = this.dialog.querySelector("#dev-friend").value;
...
async function startGame(editorApp) {
    ...
    const app = editorApp ?? new Application(canvas, {
    ...
    const sun = editorApp?.root.findByTag("migration.sun")[0] ?? new Entity("Soft afternoon sunlight", app);
    if (!sun.light) sun.addComponent("light", {
        type: "directional",
        color: new Color(1, 0.92, 0.83),
        intensity: 1.2,
        castShadows: true,
        shadowResolution: 1024,
        shadowDistance: 35,
    ...
    app.on("update", (elapsed)=>{
        houseMusic.update({
            mode: loop.mode,
            phase: props.daily.clock.state.phase,
            store: loop.mode === "store" ? loop.store.definition.id : "",
            paused: loop.popUI.isOpen || loop.developerPaused || tornado.active,
        ...
        dogRoaming.update(dt, loop.mode === "cleanup" && !tornado.active && !!props.pet?.loaded, [
            character.player.getPosition(),
            lilah.root.getPosition(),
            marc.root.getPosition()
        ]);
    ...
    {
        Object.defineProperty(window, "__roomTest", {
            configurable: true,
            value: {
                characterGeometry: ()=>character.animator.geometrySnapshot(),
                nearbyGeometry: ()=>app.root.findComponents("render").flatMap((r)=>r.meshInstances.filter((m)=>m.node.enabled && m.aabb.center.distance(character.player.getPosition()) < 3).map((m)=>({
                                name: m.node.name,
                                parent: m.node.parent?.name,
                                center: m.aabb.center.toArray(),
                                half: m.aabb.halfExtents.toArray()
                            }))),
                furnitureGeometry: ()=>[
                        "Art tableRound",
                        "Art loungeChairUpright",
                        "Art crib"
                    ].map((name)=>{
                        ...
                        return {
                            ...
                            meshes: n?.findComponents("render").flatMap((r)=>r.meshInstances.map((m)=>({
                                        center: m.aabb.center.toArray(),
                                        half: m.aabb.halfExtents.toArray()
                                    })))
                        };
                ...
                snapshot: ()=>({
                        ...
                        loop: loop.snapshot(),
                        obstacles: room.obstacles.map((box)=>({
    ...
    void Promise.resolve().then(()=>(init_DeveloperPanel(), DeveloperPanel_exports)).then(({ DeveloperPanel: DeveloperPanel2 })=>{
        if (!disposed) developerPanel = new DeveloperPanel2(loop, ()=>({
                fps: app.stats.frame.fps,
                drawCalls: app.stats.drawCalls.total,
                position: character.player.getPosition().toArray()
            }));
...
<html lang="en">\r
  <head>\r
    ...
    <meta name="theme-color" content="#ede6f4" />\r
    <meta name="description" content="A little room, a little adventure. Explore Arianna's cozy 3D bedroom." />\r
    <link rel="icon" type="image/svg+xml" href="/favicon.svg" />\r
    <title>Arianna \xB7 A little room</title>\r
  </head>\r
  <body>\r
    <main id="game" aria-label="Arianna's bedroom game">\r
      <canvas id="game-canvas" aria-label="3D bedroom. Move Arianna with the joystick or arrow keys." tabindex="0"></canvas>\r
      <div id="home-vignette" hidden aria-hidden="true"></div>\r
      <header class="topbar">\r
        <div class="wordmark"><span class="flower" aria-hidden="true">\u273F</span><span>Arianna<small>LITTLE EVERYDAY ADVENTURES</small></span></div>\r
        <div class="mission-stats"><span id="mission-clock" aria-label="Time remaining">1:00</span><span class="allowance-label">ALLOWANCE <b id="allowance">$0</b></span><span id="trip-wallet" hidden>WALLET <b id="trip-balance">$0</b></span></div>\r
      </header>\r
      <section class="room-title" aria-label="Current room">\r
        <span class="eyebrow"><i></i> <span id="scene-kicker">ONE COZY MINUTE</span> <span id="task-count">0 / 5</span></span>\r
        <h1>Home, sweet home.</h1>\r
        <ul id="task-list" class="task-list" aria-label="Cleanup tasks"></ul>\r
        <p id="day-label">Day 1 \xB7 Morning</p><div id="mission-picker" aria-label="Choose a cleanup mission"><button id="mission-day" type="button" aria-pressed="true">Daily life</button><button id="mission-house" type="button" aria-pressed="true">House \xB7 6</button><button id="mission-bedroom" type="button" aria-pressed="false">Bedroom \xB7 5</button><button id="mission-pet" type="button" aria-pressed="false">Puppy \xB7 1</button><button id="mission-practice" type="button" aria-pressed="false">Explore</button></div>\r
        <p id="room-connections"></p>\r
        <p id="scene-subtitle" hidden></p>\r
      </section>\r
      <div id="player-label" class="player-label" aria-hidden="true">Arianna <span>\u2661</span></div>\r
      <div id="cleanup-effects" aria-hidden="true"></div>\r
      <div id="house-doors" aria-hidden="true"></div>\r
      <div id="store-markers" hidden><span id="shop-display-marker" class="cleanup-marker">\u{1F381} Blind boxes</span><span id="shop-exit-marker" class="cleanup-marker">\u2302 Go Home</span></div>\r
      <div id="reveal-copy" hidden aria-live="polite"></div>\r
      <div id="save-message" role="status" hidden></div>\r
      <div class="move-tip cleanup-tip" id="move-tip"><span id="cleanup-hint">Move to start \xB7 60 seconds \xB7 $1 per task</span></div>\r
      <div id="cleanup-announcement" class="sr-only" role="status" aria-live="polite"></div>\r
      <div class="controls">\r
        <div class="joystick-group">\r
          <div id="joystick" role="group" aria-label="Movement joystick. Drag to move. Arrow keys or WASD also work.">\r
            <span class="direction up" aria-hidden="true">\u2303</span><span class="direction right" aria-hidden="true">\u203A</span><span class="direction down" aria-hidden="true">\u2304</span><span class="direction left" aria-hidden="true">\u2039</span>\r
            <div id="joystick-knob"><svg viewBox="0 0 32 32" aria-hidden="true"><path d="M16 5v22M5 16h22M12 9l4-4 4 4M12 23l4 4 4-4M9 12l-4 4 4 4M23 12l4 4-4 4"/></svg></div>\r
          </div>\r
          <span class="control-label">LET\u2019S EXPLORE</span>\r
        </div>\r
        <div class="action-group">\r
          <button id="action-button" type="button" disabled><span id="action-icon" aria-hidden="true">\u270B</span><strong id="action-title">Action</strong><small id="action-detail">Come closer</small></button>\r
          <span class="control-label">ACTION \xB7 SPACE / E</span>\r
        </div>\r
      </div>\r
      <section id="school-transition" hidden><span>\u{1F392}</span><h2>A little time at school</h2><p>See you after class!</p></section><footer><span class="build-badge">WALLET <b id="wallet">$0</b></span><a class="asset-credits" href="./asset-credits.html" target="_blank" rel="noopener">Art credits</a><button id="collection-button" type="button">Collection \xB7 0 / 8</button></footer>\r
      <dialog id="results" aria-labelledby="results-title" aria-describedby="results-summary">\r
        <div class="results-flower" aria-hidden="true">\u273F</div><span class="eyebrow">A LITTLE HELP GOES A LONG WAY</span>\r
        <h2 id="results-title">Nice helping!</h2><p id="results-summary"></p>\r
        <div class="results-totals"><div><strong id="results-tasks">0 / 5</strong><span>tasks completed</span></div><div><strong id="results-money">$0</strong><span>allowance earned</span></div></div>\r
        <p id="results-bonus" hidden></p><ul id="results-list" aria-label="Completed tasks"></ul>\r
        <p id="results-wallet"></p><button id="go-shopping" class="loop-button pink-button" type="button">Go Shopping \u2192</button>\r
        <button id="replay" type="button" autofocus>Play again <span aria-hidden="true">\u21BB</span></button>\r
      </dialog>\r
      <dialog id="collection-dialog" aria-labelledby="collection-title">\r
        <span class="eyebrow">LITTLE FRIENDS, LITTLE JOYS</span><h2 id="collection-title">Your dumplings</h2>\r
        <p id="collection-summary"></p><div id="collection-grid"></div>\r
        <button id="open-next" class="loop-button pink-button" type="button">Open a box</button>\r
        <button id="back-cleanup" class="loop-button" type="button">Back to cleanup \u21BB</button>\r
      </dialog>\r
      <div id="loading" role="status"><span class="loading-flower">\u273F</span><p>Making room for you\u2026</p></div>\r
      <div id="error" role="alert" hidden><h2>We couldn\u2019t open your room.</h2><p>Please use a browser with WebGL 2 enabled, then refresh.</p></div>\r
    </main>\r
...
_define_property(FullGame, "scriptName", "fullGame");

export { FullGame, startEditorGame };
