...
await mkdir('artifacts/family-boundaries',{recursive:true});
...
async function photo(name){await page.screenshot({path:'artifacts/family-boundaries/'+name+'.png'})}
let world,obstacles;
function free(x,z){
 const radius=.265;
...
try {
 await context.addInitScript(()=>{if(!sessionStorage.getItem('family-priority')){localStorage.setItem('arianna.lilah.v1',JSON.stringify({day:1,messes:[{x:.5,z:2.05,done:false}]}));sessionStorage.setItem('family-priority','1');}});
 await page.goto('http://127.0.0.1:5173');await ready();await setWorld();
 await moveTo(.4,1.4);await sleep(11000);
 assert.equal((await snap()).cleanup.daily.lilah.messes[0].done,false);assert.equal((await snap()).marc.cleaned,0);pass('Dad gives nearby Arianna priority over an existing mess');
 await approach('lilah-mess-0');await press('lilah-mess-0');let s=await snap();assert.equal(s.cleanup.daily.lilah.messes[0].cleanedBy,'arianna');assert.equal(s.loop.balance,1);await photo('01-arianna-cleans');pass('Arianna still cleans and earns exactly one allowance reward');
 ...
 await page.addInitScript(saved=>{if(!sessionStorage.getItem('family-night')){localStorage.setItem('arianna.daily.v1',JSON.stringify(saved));sessionStorage.setItem('family-night','1');}},saved);
 await page.reload();await ready();await setWorld();await page.waitForFunction(()=>window.__roomTest.snapshot().lighting.amount===1);
 s=await snap();assert.equal(s.lighting.lights.length,13);assert.ok(s.lighting.lights.every(l=>l.enabled&&l.intensity>0));assert.equal(s.loop.balance,1);
 await moveTo(7.95,3.5);await page.waitForFunction(()=>{const l=window.__roomTest.snapshot().lilah;return l.state==='sleepy'&&l.position[0]>6.5&&l.position[2]<5.5&&l.routeLength===0;},undefined,{timeout:45000});await photo('02-nursery-night');
 await moveTo(7.85,8.85);await photo('03-marc-room-night');await moveTo(-1.25,15.3);await photo('04-kitchen-additions');pass('Both new bedroom lamps work at night; Lilah reaches her own nursery');
 const count=(await snap()).cleanup.daily.lilah.messes.length;await sleep(1000);assert.equal((await snap()).cleanup.daily.lilah.messes.length,count);
 for(const id of ['daily-teeth','night-clothes','clothes-drawer','bedtime-book','sleep']){if(id==='night-clothes')await moveTo(-.65,-.25);else await approach(id);await press(id);}
 s=await snap();assert.equal(s.cleanup.daily.day,2);assert.equal(s.cleanup.daily.lilah.messes.length,0);assert.equal(s.loop.balance,4);pass('Night routine and next day reset remain intact with no duplicated mess reward');
 assert.deepEqual(errors,[]);await writeFile('artifacts/family-boundaries/report.json',JSON.stringify({checks,errors,snapshot:s},null,2));
} catch(error){await photo('failure');await writeFile('artifacts/family-boundaries/failure.json',JSON.stringify({error:String(error),errors,snapshot:await snap().catch(()=>null)},null,2));console.error(error);process.exitCode=1;}finally{await browser.close();}
