...
const page=await context.newPage(),errors=[],rounds=[];await mkdir('artifacts/dog-rig',{recursive:true});
...
try{
 ...
 for(const special of ['dog']){
   ...
   while((s=await snap()).tornado.phase==='playing'){
     assert.ok(s.tornado.messes.length<=3);stats.push({time:s.tornado.elapsed,fps:s.fps,drawCalls:s.drawCalls,position:s.lilah.position,dog:s.dog});
     ...
     if(mess&&!s.tornado.cleaning){await moveTo(mess.point);s=await snap();if(s.tornado.phase==='playing'&&!(await page.locator('#action-button').isDisabled())){await page.locator('#action-button').tap();await sleep(650);if((await snap()).cameraState==='CHORE')await page.screenshot({path:`artifacts/dog-rig/${special}-chore-${s.tornado.cleaned}.png`});await sleep(1500);}}
     else await sleep(180);
     s=await snap();if(s.tornado.elapsed>nextPhoto&&s.tornado.phase==='playing'){await page.screenshot({path:`artifacts/dog-rig/${special}-${nextPhoto}.png`});nextPhoto+=15;}
   }
   assert.ok(stats.some(t=>t.dog?.state==='Walk'),'Dog used the skeletal walk');assert.ok(stats.every(t=>Math.abs(t.dog?.height-.48)<.0001),'Dog keeps original height');assert.equal(s.tornado.paid,true);assert.equal(s.loop.balance,balance+s.tornado.reward);assert.ok(s.tornado.cleaned>=3,'At least 3 real cleanups');
   if(special!=='none')assert.ok(seen.has(special),'Visible '+special+' interruption');
   assert.equal(s.tornado.elapsed,55);await page.screenshot({path:`artifacts/dog-rig/${special}-result.png`});rounds.push({special,seen:[...seen],result:s.tornado,stats,dayBefore:day,dayAfter:s.cleanup.daily});console.log('ROUND',special,JSON.stringify(s.tornado));
   await page.locator('#tornado-dialog [data-action=back]').click();await sleep(1200);s=await snap();assert.equal(s.cameraState,'EXPLORE');assert.equal(s.tornado.phase,'idle');assert.ok(Math.abs(s.cameraHeight-5.735)<.1);assert.equal(s.character.busy,false);assert.equal(s.dog.state,'Idle');assert.deepEqual(s.dog.position,initial.dog.position);
 }
 assert.deepEqual(errors,[]);await writeFile('artifacts/dog-rig/rounds.json',JSON.stringify({errors,rounds},null,2));
}catch(error){await page.screenshot({path:'artifacts/dog-rig/failure.png'});await writeFile('artifacts/dog-rig/failure.json',JSON.stringify({error:String(error),errors,rounds,snapshot:await snap()},null,2));console.error(error);process.exitCode=1;}finally{await browser.close();}
