...
const daily={version:1,day:1,minutes:940,phase:'afternoon',done:['dust-0','dust-1','dust-2','spill','laundry-clothes'],eggDrop:false,breakfast:'done',dust:[0,1,2],schoolSeconds:0};
const progress={version:1,balance:100,collection:{},boxes:[],creditedRounds:[],trip:{active:true,purchases:0},location:'store',reveal:null,hunt:createHuntDay(1,()=>.5)};
progress.hunt.activeStore='corner';progress.hunt.stores.corner.slots=[0,1,2,3,4,5].map(site=>({site,series:site%2?'treats':'garden',remaining:2,discovered:false}));
await context.addInitScript(({progress,daily})=>{localStorage.setItem('arianna.progress.v1',JSON.stringify(progress));localStorage.setItem('arianna.daily.v1',JSON.stringify(daily));},{progress,daily});
try{
 await page.goto('http://127.0.0.1:5173',{waitUntil:'domcontentloaded'});await page.waitForFunction(()=>window.__roomTest?.snapshot().characterLoaded&&window.__roomTest.snapshot().loop.store?.art.loaded>=22);let s=await snap();world=s.loop.store.walkable;obstacles=s.loop.store.obstacles;
 for(const id of [3,1,0,4,5,2]){s=await snap();const site=s.loop.store.sites.find(x=>x.id===id);await moveTo(site.position[0],site.position[2]);await press('hunt-site-'+id);await page.waitForFunction(id=>window.__roomTest.snapshot().loop.hunt.stores.corner.slots.find(s=>s.site===id).discovered,id);await photo('visible-boxes-'+id);}
 assert.deepEqual((await snap()).loop.store.art.errors,[]);pass('All six final stock locations loaded, inspected and photographed with two boxes each');
 for(const size of [{width:320,height:568},{width:430,height:932}]){
  await page.setViewportSize(size);await sleep(350);await photo('phone-find-'+size.width);
  const panel=await page.locator('#hunt-find').boundingBox(),button=await page.locator('#action-button').boundingBox();assert.ok(panel.y+panel.height<=button.y+2,'Find card overlaps Action');
  assert.ok(panel.x>=0&&panel.x+panel.width<=size.width);assert.ok(button.y+button.height<=size.height);assert.equal(await page.locator('#move-tip').isVisible(),false);
 }
 pass('Series card and Action button remain separate on 320px and 430px phones');
 await page.setViewportSize({width:390,height:844});await sleep(200);const old=(await snap()).loop.balance;await press('hunt-site-2');await sleep(150);assert.equal((await snap()).loop.balance,old-4);await photo('final-purchase-feedback');assert.equal(await page.locator('#save-message').isVisible(),true);
 const panel=await page.locator('#hunt-find').boundingBox(),toast=await page.locator('#save-message').boundingBox();assert.ok(toast.y+toast.height<panel.y);pass('Purchase feedback and series details are both readable');
 const exit=(await snap()).loop.store.exit;await moveTo(exit[0],exit[2]-.1);await press('go-home');await press('open-box');await page.waitForFunction(()=>window.__roomTest.snapshot().loop.phase==='revealed');await press('collection');await page.locator('#back-cleanup').click();s=await snap();world=s.walkable;obstacles=s.obstacles;await approach('shop-door');await press('shop-door');
 for(const size of [{width:320,height:568},{width:430,height:932}]){await page.setViewportSize(size);await sleep(150);const dialog=await page.locator('#hunt-routes').boundingBox();assert.ok(dialog.x>=0&&dialog.width<=size.width&&dialog.y>=0&&dialog.height<=size.height);await photo('phone-routes-'+size.width);}
 pass('Three-store chooser fits short and tall portrait screens');assert.deepEqual(errors,[]);
 await writeFile('artifacts/squishy-hunt/layout.json',JSON.stringify({checks,errors,snapshot:await snap()},null,2));
}catch(error){await photo('layout-failure');await writeFile('artifacts/squishy-hunt/layout-failure.json',JSON.stringify({error:String(error),errors},null,2));console.error(error);process.exitCode=1}finally{await browser.close()}
