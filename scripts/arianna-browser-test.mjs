...
const checks = [], errors = [];
await mkdir('artifacts/arianna', { recursive: true });
...
async function createPlayer(name) {
  ...
  async function screenshot(file) { await page.screenshot({ path: `artifacts/arianna/${file}.png` }); }
...
try {
  ...
  assert.equal(initial.character.scale, 1);
  assert.deepEqual(initial.character.materials, ['Arianna | opaque vertex palette']);
  assert.equal(initial.character.clips.length, 8);
  for (const clip of initial.character.clips) assert.equal(clip.loop, !['PickUp','PutDown','Celebrate'].includes(clip.name));
  pass('Approved GLB imports at authored scale with the vertex-color material and all eight loop/one-shot states');
  const observations = [];
  const geometry = await page.evaluate(() => window.__roomTest.characterGeometry());
  assert.equal(geometry[0].joints,25); assert.equal(geometry[0].vertices,3294); assert.equal(geometry[0].vertexColors,true);
  assert.ok(Math.abs(geometry[0].minY-initial.character.groundY)<.005, JSON.stringify(geometry));
  assert.ok(Math.abs(geometry[0].maxY-geometry[0].minY-1.203)<.025);
  observations.push({idleGeometry:geometry});
  pass('Actual skinned mesh retains vertex colors, 25 joints and 1.203m height; shoe soles contact the rug');
  async function action(expected, clip, eventTime, before, after) {
    ...
    const origin = (await snap()).position;
    ...
    assert.ok(event.time >= eventTime && event.time < eventTime+.08, JSON.stringify(event));
    assert.ok(samples.at(-1).character.clipTime > 2.35);
    observations.push({clip,event,samples:samples.length,finalClipTime:samples.at(-1).character.clipTime});
    pass(clip + ' plays the full 2.4s, faces its target, locks translation and commits once at ' + eventTime + 's');
  }
  await moveTo(.15,-.55);
  await action('pickup-book','PickUp',1.1,s=>s.carrying===null,s=>s.carrying==='book');
  await sleep(230);
  ...
  async function walkSample(carrying) {
    const c=await center('#joystick');
    await touch('touchStart',1,c);
    ...
    await screenshot(carrying?'03-carry-walk':'05-walk');
    await touch('touchEnd',1);
    const moving=samples.filter(s=>Math.hypot(...s.velocity)>2);
  ...
  await action('bookshelf','PutDown',1.3,s=>s.carrying==='book',s=>s.carrying===null);
  ...
  await writeFile('artifacts/arianna/animation-report.json',JSON.stringify({checks,errors,observations},null,2));
