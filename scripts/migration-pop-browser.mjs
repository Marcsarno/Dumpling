...
const cdp=await context.newCDPSession(page),pop=()=>page.evaluate(()=>window.__roomTest.snapshot().loop.pop),save=()=>page.evaluate(()=>JSON.parse(localStorage.getItem('dumpling.editorMigration.progress.v1')));
...
try{
 ...
 assert.deepEqual(errors,[]);await writeFile('migration/evidence/pop-levels/report.json',JSON.stringify({rounds,errors,replayRetained:true,classic:true},null,2));console.log('PASS reload/unlocks, idle replay retention, Classic, touch, mobile layout and clean console');
