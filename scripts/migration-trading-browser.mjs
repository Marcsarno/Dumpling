...
await mkdir('migration/evidence/trading',{recursive:true});
page.on('pageerror',e=>errors.push(e.message));page.on('console',m=>{if(['error','warning'].includes(m.type())&&!m.location().url.endsWith('/favicon.ico'))errors.push(m.text())});
page.on('response',r=>{if(r.status()>=400)errors.push(r.status()+' '+r.url())});
const snap=()=>page.evaluate(()=>window.__roomTest.snapshot());
const sleep=ms=>new Promise(r=>setTimeout(r,ms));
const pass=name=>{checks.push(name);console.log('PASS '+name)};
const photo=name=>page.screenshot({path:'migration/evidence/trading/'+name+'.png'});
...
await context.addInitScript(progress=>{if(!localStorage.getItem('trading-test-started')){localStorage.setItem('dumpling.editorMigration.progress.v1',JSON.stringify(progress));localStorage.setItem('trading-test-started','1');}},progress);
try{
  ...
  await page.addInitScript(()=>{if(sessionStorage.getItem('trading-school-fixture'))return;sessionStorage.setItem('trading-school-fixture','1');const d={version:1,day:2,minutes:510,phase:'school',done:[],eggDrop:false,breakfast:'done',dust:[0,1,2],schoolSeconds:3};localStorage.setItem('dumpling.editorMigration.daily.v1',JSON.stringify(d));const p=JSON.parse(localStorage.getItem('dumpling.editorMigration.progress.v1'));p.location='cleanup';localStorage.setItem('dumpling.editorMigration.progress.v1',JSON.stringify(p));});
  ...
  assert.deepEqual(errors,[]);await writeFile('migration/evidence/trading/report.json',JSON.stringify({checks,errors,snapshot:await snap()},null,2));
}catch(error){await photo('failure');console.error(error);await writeFile('migration/evidence/trading/failure.json',JSON.stringify({error:String(error),errors,snapshot:await snap().catch(()=>null)},null,2));process.exitCode=1;}finally{await browser.close();}
