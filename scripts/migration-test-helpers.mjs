...
await mkdir('migration/evidence/scene',{recursive:true});const errors=[],checks=[];page.on('pageerror',e=>errors.push(e.message));page.on('response',r=>{if(r.status()>=400)errors.push(r.status()+' '+r.url());});
...
progress.hunt.stores.corner.slots[0].site=5;
...
const photo=n=>page.screenshot({path:`migration/evidence/scene/${n}.png`});
...
export function setPlanner(world){planner=new HousePath({walkable:world.walkable,obstacles:world.obstacles.map(o=>new BoundingBox(new Vec3(...o.center),new Vec3(...o.halfExtents)))},.265);}
