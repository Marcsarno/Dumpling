...
try{
 await page.goto('http://127.0.0.1:5173');await page.waitForFunction(()=>window.__roomTest?.snapshot().cleanup.pet?.loaded);
 let s=await snap();world=s.walkable;obstacles=s.obstacles;await page.locator('#mission-pet').tap();
 await approach('scoop-poop');assert.equal(await page.locator('#action-button').isDisabled(),true);pass('Bare hands cannot collect poop');
 await approach('pickup-scooper');await press('pickup-scooper');await approach('flush-poop');assert.equal(await page.locator('#action-button').isDisabled(),true);pass('Empty scooper cannot flush; washing cannot skip the task');
 await approach('scoop-poop');await press('scoop-poop');await approach('flush-poop');await press('flush-poop');
 assert.equal((await snap()).cleanup.completed.length,0);assert.equal((await snap()).cleanup.allowance,0);
 console.log('Waiting for real mission expiry before washing...');await page.locator('#results[open]').waitFor({timeout:65000});s=await snap();assert.equal(s.cleanup.reason,'time');assert.equal(s.cleanup.allowance,0);assert.equal(s.cleanup.completed.length,0);pass('Flushing alone never completes or rewards the task, including real 60-second expiry');
 await page.locator('#replay').tap();await page.locator('#mission-practice').tap();await approach('pickup-scooper');await press('pickup-scooper');await approach('scoop-poop');await press('scoop-poop');await approach('flush-poop');await press('flush-poop');
 await approach('pickup-bath-towel');assert.notEqual(await page.locator('#action-button').getAttribute('data-target'),'pickup-bath-towel');pass('Hands must be washed before another chore in Explore');
 await approach('wash-hands');await press('wash-hands');await page.waitForFunction(()=>window.__roomTest.snapshot().cleanup.pet.stage==='done');assert.equal((await snap()).cleanup.allowance,0);await moveTo(4.55,-1.25);await press('pickup-bath-towel');assert.equal((await snap()).cleanup.carrying,'bath-towel');pass('Washing unlocks normal carrying; Explore awards no money');
 await page.locator('#mission-pet').tap();for(const [width,height] of [[320,568],[360,640],[430,932],[844,390]]){await page.setViewportSize({width,height});await sleep(150);for(const selector of ['#mission-pet','#joystick','#action-button']){const b=await page.locator(selector).boundingBox();assert.ok(b.x>=0&&b.y>=0&&b.x+b.width<=width&&b.y+b.height<=height)}await photo('pet-'+width+'x'+height)}pass('Puppy selector and controls fit small phones and landscape');
 assert.deepEqual(errors,[]);await writeFile('artifacts/pets-store/edge-report.json',JSON.stringify({checks,errors},null,2));
