...
const output='migration/evidence/reveals';await mkdir(output,{recursive:true});
const browser=await chromium.launch({channel:'msedge',headless:true}),report=[];
const cases=[['rosie',false,390,844],['mochi',true,320,568],['blueberry',false,390,844],['cocoa',true,320,568],['peachy',false,390,844],['lavendream',true,320,568],['supernova',false,390,844],['stardrop',true,320,568],['supernova',true,430,932,true]];
try{for(const[id,duplicate,width,height,reduced=false] of cases){
 ...
 await context.addInitScript(({id,duplicate})=>{if(localStorage.getItem('polish-fixture'))return;localStorage.setItem('polish-fixture','1');localStorage.setItem('dumpling.editorMigration.progress.v1',JSON.stringify({version:1,balance:27,collection:duplicate?{[id]:2}:{},boxes:[{id:'polish-box',dumplingId:id}],creditedRounds:['old-round'],trip:{active:false,purchases:0},location:'home',reveal:null,pop:{tickets:11,bestScore:900,rounds:['old-pop'],tutorialSeen:true,couponDay:0}}));},{id,duplicate});
 const snapshot=()=>page.evaluate(()=>window.__roomTest.snapshot()),save=()=>page.evaluate(()=>JSON.parse(localStorage.getItem('dumpling.editorMigration.progress.v1')));
 try{
  await page.goto(process.env.GAME_URL??'http://127.0.0.1:5178');await page.locator('#game[data-ready=true]').waitFor({timeout:60000});await page.waitForTimeout(350);
