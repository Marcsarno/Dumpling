...
const c=new DailyClock(()=>.25);assert.equal(c.label,'7:00 AM');c.advance(120);assert.equal(c.label,'8:00 AM');assert.equal(c.canShop,false);c.advance(60);assert.equal(c.schoolDue,true);assert.equal(c.goSchool(),true);c.advance(3);assert.equal(c.state.phase,'afternoon');assert.equal(c.canShop,true);c.advance(480);assert.equal(c.state.phase,'night');assert.equal(c.canShop,true);c.advance(240);assert.equal(c.sleep(),true);assert.equal(c.state.day,2);assert.equal(c.state.phase,'morning');
