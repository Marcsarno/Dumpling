...
try{
 await page.goto('http://127.0.0.1:5173');await page.waitForFunction(()=>window.__roomTest?.snapshot().cleanup.pet?.loaded,undefined,{timeout:20000});
 let s=await snap();world=s.walkable;obstacles=s.obstacles;
 await page.locator('#mission-pet').tap();await approach('pickup-scooper');await photo('01-tool');await press('pickup-scooper');
 assert.equal((await snap()).cleanup.pet.stage,'scoop');
 await approach('scoop-poop');await photo('02-pug-poop');await press('scoop-poop');await photo('03-loaded-scooper');
 assert.equal((await snap()).cleanup.pet.stage,'flush');assert.equal((await snap()).cleanup.pet.poopParent,'Pooper scooper');assert.equal((await snap()).cleanup.allowance,0);
 await approach('flush-poop');await photo('04-toilet');await press('flush-poop');assert.equal((await snap()).cleanup.pet.stage,'wash');assert.equal((await snap()).cleanup.allowance,0);assert.equal((await snap()).cleanup.carrying,null);await photo('05-wash-next');
 await approach('wash-hands');await press('wash-hands');await sleep(500);await photo('06-washing');await page.locator('#results[open]').waitFor();s=await snap();assert.equal(s.cleanup.completed.length,1);assert.equal(s.cleanup.allowance,3);await photo('07-results');pass('Scoop, carry, flush, wash completes in order; only final wash awards $1 plus $2 bonus');
 await page.locator('#replay').tap();assert.equal((await snap()).cleanup.pet.stage,'tool');pass('Replay restores mess and scooper');
 assert.deepEqual(errors,[]);await writeFile('artifacts/pets-store/report.json',JSON.stringify({checks,errors,snapshot:s},null,2));
