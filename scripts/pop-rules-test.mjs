...
for(let i=0;i<10;i++)save.completePopRound('big'+i,9999);assert.equal(save.popDiscount(),1);const slot=save.data.hunt.stores.corner.slots.find(s=>s.remaining);save.discover(slot.site);const before=save.data.balance,tickets=save.data.pop.tickets;save.purchaseStock(slot.site);assert.equal(before-save.data.balance,boxPrice(storeById('corner'),slot.series)-1);assert.equal(tickets-save.data.pop.tickets,40);assert.equal(save.popDiscount(),0);save.ensureHuntDay(2);assert.equal(save.popDiscount(),1);assert.equal(new ProgressStore(repo).data.pop.tickets,46);
console.log('PASS power thresholds/radii/cascades, wildcard/backtrack, Frenzy, deadlock, collection pool, old saves, state preservation, idempotent rewards, failed writes and daily coupon cap');
