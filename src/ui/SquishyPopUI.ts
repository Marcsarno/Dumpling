...
import { POP_LEVELS, PopLevelRun, createLevelBoard, levelStars, levelUnlocked, type PopLevel, type PopLevelAttempt, type PopLevelRecords, type PopObjective } from '../data/popLevels';
...
export class SquishyPopUI {
  ...
  private state:'levels'|'briefing'|'intro'|'lesson'|'countdown'|'playing'|'finale'|'results'='levels';
  private level?:PopLevel;private levelRun?:PopLevelRun;private goalMarkup='';
  ...
  constructor(private collection:()=>Record<string,number>,private award:(id:string,score:number,attempt?:PopLevelAttempt)=>number,private onOpen:(open:boolean)=>void,private firstPlay:()=>boolean=()=>true,private record:()=>{bestScore:number;tickets:number;levels?:PopLevelRecords}=()=>({bestScore:0,tickets:0})){
    ...
    this.dialog.addEventListener('pointerdown',()=>{void this.audio.unlock();},{signal});
    this.dialog.addEventListener('keydown',()=>{void this.audio.unlock();},{signal});
    ...
    this.dialog.querySelector('.pop-sound')!.addEventListener('click',()=>{void this.audio.unlock();const muted=this.audio.mute();this.q('.pop-sound').textContent=muted?'♫̸':'♪';this.q('.pop-sound').setAttribute('aria-label',muted?'Unmute sound':'Mute sound');},{signal});
  ...
  private chooseLevels(){this.cancel();this.paused=false;this.state='levels';this.audio.setMusic(false);this.showCover();}
  private objective(o:PopObjective,live=false,i=0){
    const value=Math.min(this.levelRun?.values[i]??0,o.target),count=live?`${value} / ${o.target}`:String(o.target);
    const icon=(power:string)=>`<img src="${this.art.powers.get(power)!.toDataURL()}" alt="">`;
    if(o.kind==='score')return `✦ ${count} points`;
    if(o.kind==='chains')return `🔗 ${count} chains of ${o.length}+`;
    if(o.kind==='friend'){const id=this.board.pool[o.slot],name=DUMPLINGS.find(d=>d.id===id)!.name;return `<img src="${this.images.get(id)!.toDataURL()}" alt="${name}"> Pop ${count}`;}
  ...
  private updateGoals(){
    ...
    if(markup!==this.goalMarkup){this.goalMarkup=markup;this.q('.pop-goals').innerHTML=markup;}this.q('.pop-goals').hidden=!markup;
  }
  ...
  private async prepare(){const version=++this.prepareVersion;this.prepared=false;this.q('.pop-cover').hidden=false;this.q('.pop-cover').innerHTML='<h3>Gathering squishy friends…</h3>';try{await this.art.load();if(!this.isOpen||version!==this.prepareVersion)return;this.prepared=true;this.level=undefined;this.start();this.chooseLevels();this.last=performance.now();this.frame=requestAnimationFrame(this.tick);}catch{if(!this.isOpen||version!==this.prepareVersion)return;this.q('.pop-cover').innerHTML='<h3>Friends need a moment.</h3><button data-retry>Try again</button><button data-back>Back to game</button>';this.q('[data-retry]').onclick=()=>void this.prepare();this.q('[data-back]').onclick=()=>this.close();}}
  private start(){
    this.board=this.level?createLevelBoard(this.level,{...this.collection()}):new PopBoard(boardPool(this.collection()),{...this.collection()});this.levelRun=this.level?new PopLevelRun(this.level,this.board.pool):undefined;this.remaining=this.level?.seconds??POP_RULES.seconds;this.activeTime=0;this.countdown=3.8;this.paused=false;this.saved=false;this.receipt=crypto.randomUUID();this.moves.clear();this.particles.forEach(p=>p.life=0);this.lockedUntil=0;this.feedbackUntil=0;
    ...
    this.state=this.level?'briefing':this.firstPlay()?'intro':'countdown';this.cancel();
    ...
    this.updateGoals();this.showCover();this.resize();if(this.state==='countdown')this.audio.setMusic(true);
  ...
  private showCover(){
    ...
    if(this.paused){cover.innerHTML='<h3>A little breather ♡</h3><button data-resume>Keep popping</button><button data-quit>Back to game</button>';cover.querySelector('[data-resume]')!.addEventListener('click',()=>this.togglePause());cover.querySelector('[data-quit]')!.addEventListener('click',()=>this.close());}
    else if(this.state==='levels'){
      cover.innerHTML='<h3>Your squishy trail</h3><p>1 → 2 → 3 · A new goal each round</p><div class="pop-level-list">'+POP_LEVELS.map((level,i)=>{const r=this.record().levels?.[level.id],unlocked=levelUnlocked(level,this.record().levels);return '<button data-level="'+i+'" '+(unlocked?'':'disabled')+'><b>'+ (unlocked?(i+1):'🔒')+' · '+level.name+'</b><small>'+(r?.completed?'★'.repeat(r.stars)+' · Best '+r.bestScore:unlocked?'Ready to play':'Finish Level '+i+' to unlock')+'</small></button>';}).join('')+'</div><button data-classic>Classic · 60 seconds</button><button class="pop-link" data-back>Back to game</button>';
      cover.querySelectorAll<HTMLButtonElement>('[data-level]').forEach(b=>b.onclick=()=>{const level=POP_LEVELS[Number(b.dataset.level)];if(!levelUnlocked(level,this.record().levels))return;this.level=level;this.start();});
      this.q('[data-classic]').onclick=()=>{this.level=undefined;this.start();};this.q('[data-back]').onclick=()=>this.close();
    }
    else if(this.state==='briefing'){
      cover.innerHTML='<small>LEVEL '+(POP_LEVELS.indexOf(this.level!)+1)+' · '+this.remaining+' seconds</small><h3>'+this.level!.name+'</h3><div class="pop-objectives">'+this.level!.objectives.map(o=>'<span>'+this.objective(o)+'</span>').join('')+'</div><p>'+this.level!.hint+'</p><small>★ Finish the goal · ★★ '+this.level!.stars[0]+' points · ★★★ '+this.level!.stars[1]+'</small><button data-play-level>Let’s pop!</button><button class="pop-link" data-levels>Choose a level</button>';
      this.q('[data-play-level]').onclick=()=>{this.state=this.firstPlay()?'intro':'countdown';this.audio.setMusic(this.state==='countdown');this.showCover();};this.q('[data-levels]').onclick=()=>this.chooseLevels();
    }
    else if(this.state==='intro'){cover.innerHTML=`<h3>Little chains.<br>Big squishes!</h3><div class="pop-demo">${Array.from({length:3},()=>`<img src="${this.images.get(this.board.pool[0])!.toDataURL()}" alt="">`).join('')}<b>☝</b></div><p>Let’s try one together. No timer yet!</p><button data-start>Show me how</button><button class="pop-link" data-back>Back to game</button>`;cover.querySelector('[data-start]')!.addEventListener('click',()=>{void this.audio.unlock();this.state='lesson';[12,13,14].forEach(i=>this.board.pieces[i].kind=this.board.pool[0]);this.q('.pop-hint').textContent='Follow the glow: connect these 3 friends';this.showCover();});cover.querySelector('[data-back]')!.addEventListener('click',()=>this.close());}
  ...
  private results(){
    ...
    if(this.level&&this.levelRun){
      ...
      cover.querySelector('.pop-previous-best')!.innerHTML=this.level.objectives.map((o,i)=>this.objective(o,true,i)).join(' · ');
      const next=POP_LEVELS[POP_LEVELS.indexOf(this.level)+1];
      if(this.saved&&!this.developerPractice&&won&&next){const button=document.createElement('button');button.dataset.nextLevel='';button.textContent='Next · '+next.name;button.onclick=()=>{this.level=next;this.start();};cover.querySelector('[data-replay]')!.before(button);}
      cover.querySelector('[data-replay]')!.textContent=error?'Retry saving':won?'Replay this level':'Try again · you keep your tickets';
    }
    if(this.saved){const button=document.createElement('button');button.className='pop-link';button.dataset.levels='';button.textContent='Choose a level';button.onclick=()=>this.chooseLevels();cover.querySelector('[data-back]')!.before(button);}
    ...
    const actions=document.createElement('div');actions.className='pop-result-actions';cover.querySelectorAll('button').forEach(button=>actions.append(button));cover.append(actions);
  }
  ...
  private togglePause(){if(!this.isOpen||['results','intro','levels','briefing'].includes(this.state))return;this.cancel();this.paused=!this.paused;this.audio.pause(this.paused);if(!this.paused)void this.audio.unlock();this.showCover();}
