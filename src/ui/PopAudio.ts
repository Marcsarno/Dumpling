...
export class PopAudio {
  ...
  pop(chain:number){this.sample(`drop_00${1+Math.floor(Math.random()*3)}`,.92+Math.random()*.16,.65);if(chain>=8){this.sample('confirmation_001',1.12,.65);[523,659,784,1047,1319,1568].forEach((n,i)=>this.note(n,i*.075,.3,.15));[262,330,392].forEach(n=>this.note(n,.42,.55,.08,'triangle'));}else if(chain>=5){[523,659,784,1047].forEach((n,i)=>this.note(n,i*.07,.26,.14));}}
  power(kind:string){if(kind==='bomb'){this.note(140,0,.3,.25,'triangle');this.sample('drop_003',.6,.7);}else if(kind==='mega'){[110,220,440,880].forEach((n,i)=>this.note(n,i*.065,.35,.2,'triangle'));}else if(kind==='rainbow'){[523,587,659,698,784,880,988,1047].forEach((n,i)=>this.note(n,i*.06,.3,.13));}else[660,880,1100,1320].forEach((n,i)=>this.note(n,i*.05,.2,.1));}
