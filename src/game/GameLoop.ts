...
import { HUNT_RULES, SERIES, STORES, boxPrice, seriesById, storeById } from '../data/hunt';
...
export class GameLoop {
    ...
    if(this.mode==='recess'){
      ...
      if(slot){
        ...
        if(this.inspecting){title='Taking a peek…';detail='What series is it?';}
          ...
          const series=seriesById(slot.series),price=boxPrice(this.store.definition,slot.series),full=data.trip.purchases>=HUNT_RULES.bagLimit,short=data.balance<price;
          ...
          copy=`${series.name}|$${price} · Owned ${owned}/${series.items.length}|${slot.remaining===1?'Last box at this display.':'A couple of boxes here.'} The friend inside is a surprise.`;
        }
  ...
  snapshot() { return { mode: this.mode, balance: this.save.data.balance, boxes: this.save.data.boxes.length, purchases: this.save.data.trip.purchases, collection: { ...this.save.data.collection }, phase: this.opening.phase, reveal: this.save.data.reveal ? { ...this.save.data.reveal } : null, focus: this.focus,
    trading:this.save.data.trading, recess:this.mode==='recess'?{seats:this.recess.seats.map(s=>({id:s.id,position:s.anchor.toArray()})),art:this.recess.artStats?.()}:null,
  ...
  destroy() { this.abort.abort(); this.action.destroy(); this.opening.destroy(); this.huntUI.destroy(); this.tradingUI.destroy(); }
}
