...
export function squishyOpeningPose(seconds:number){
 const lift=smooth((seconds-.90)/1.05),settle=Math.max(0,seconds-1.95);
 const bounce=seconds>1.95?Math.sin(settle*12)*Math.exp(-settle*6)*.048:0;
 const height=.11+.39*lift-.16*smooth((seconds-1.95)/.75)+bounce;
 const size=.51+.49*lift,stretch=1+Math.sin(lift*Math.PI)*.14+(seconds>1.95?Math.sin(settle*12)*Math.exp(-settle*7)*.075:0);
