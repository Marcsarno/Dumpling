    ...
    if(target.kind==='daily') {
      ...
      if(target.duration===0){
        ...
        if(target.id==='eat-breakfast'){
          this.seatReturn=this.character.player.getPosition().clone();this.controller.reset();this.character.player.setPosition(propPoint('dining',new Vec3(.55,.09,12.49)));this.character.visual.setLocalEulerAngles(0,propYaw('dining',0),0);this.character.animator.setCarrying(false);if(this.character.grounding)this.character.grounding.surfaceHeight=.07;this.character.animator.setWorkClip('EatSit',propPoint('dining',new Vec3(.55,1,13.85)));return;
        }
