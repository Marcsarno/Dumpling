...
import {Entity,Texture,StandardMaterial,CULLFACE_NONE,type Application} from 'playcanvas';
...
/** Original student sculpts on compatible Quaternius rigs share the same chairs and trading identities. */
export class Classmates {
 ...
 constructor(private app:Application,parent:Entity,placements?:{x:number;y?:number;z:number}[],showNames=true){
  this.ready=Promise.all(TRADERS.map(async(t,i)=>{
   ...
   try{const person=await schoolPerson(app,parent,['student-jules','student-remy','student-poppy'][i],t.name+' classmate',[1.38,1.36,1.34][i],true);person.root.setLocalPosition(x,placements?.[i].y??0,z);this.actors.push({person,id:t.id,near:false});this.loaded++;}
 ...
 snapshot(){return{loaded:this.loaded,errors:[...this.errors],people:this.actors.map(a=>({id:a.id,height:a.person.height,source:'Original student sculpt on Quaternius rig'}))};}
}
