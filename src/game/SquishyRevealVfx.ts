...
export class SquishyRevealVfx {
  ...
  private ring(radius:number,width:number,alpha:number){const start=this.vertices;for(let i=0;i<=48;i++){const a=i*Math.PI/24;for(const r of [radius-width,radius,radius+width])this.vertex(Math.cos(a)*r,Math.sin(a)*r,r===radius?alpha:0,.35);}for(let i=0;i<48;i++)for(let j=0;j<2;j++){const a=start+i*3+j;this.triangle(a,a+3,a+1);this.triangle(a+1,a+3,a+4);}}
  ...
  update(time:number,reduced:boolean){
    ...
    // A short column of rarity light follows the launch, fading before the landing.
    if(!reduced&&t<1.05){const fade=Math.sin(Math.min(1,t/1.05)*Math.PI),start=this.vertices;
      this.vertex(-.18,-.55,fade*.42,.6);this.vertex(.18,-.55,fade*.42,.6);
      this.vertex(.40,1.30,0,.8);this.vertex(-.40,1.30,0,.8);
      this.triangle(start,start+1,start+2);this.triangle(start,start+2,start+3);
    }
    ...
    if(s.sparkles&&!reduced){
      this.color.fromString(s.spark);if(burst>0){this.ring(.60+Math.min(t,1.5)*.27,s.rays?.035:.025,burst*.60);
