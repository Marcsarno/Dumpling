...
import { BoundingBox as BoundingBox3, Color as Color3, Script, Vec3 as Vec38 } from "playcanvas";

// src/components/CharacterVisual.ts
import { Asset, BoundingBox, Entity as Entity6 } from "playcanvas";

// src/components/MeshyGameplayAdapter.ts
import { AnimCurve as AnimCurve2, AnimData as AnimData2, AnimTrack as AnimTrack2, Quat as Quat2, Vec3 as Vec32, INTERPOLATION_LINEAR as INTERPOLATION_LINEAR2 } from "playcanvas";
...
import { AnimCurve, AnimData, AnimTrack, Quat, Vec3, INTERPOLATION_LINEAR } from "playcanvas";
var RUN_PALM_AXES = { Left: new Vec3(-0.084140846, 0.99499861, -0.05383385).normalize(), Right: new Vec3(-0.16291618, 0.98452002, 0.06464243).normalize() };
...
function meshyGameplay(model, source, chore) {
  ...
  const crouch = () => {
    ...
    for (const foot of feet) {
      ...
      delta.normalize();
      const pole = model.getWorldTransform().transformVector(new Vec32(0, 0, 1));
  ...
  const arms = (y, z, wide = 0.11, motion) => {
    for (const [side, sign] of [["Left", 1], ["Right", -1]]) {
      ...
      const shoulder = arm.getPosition().clone(), target = model.getWorldTransform().transformPoint(new Vec32(sign * wide + (motion?.[i] ?? 0) * 0.09, y + (motion?.[i + 1] ?? 0) * 0.055, z + (motion?.[i + 2] ?? 0) * 0.09));
      ...
      direction.normalize();
      const pole = model.getWorldTransform().transformVector(new Vec32(sign, -0.6, -0.3));
  ...
  const axis = model.getWorldTransform().transformVector(Vec32.RIGHT).normalize();
  ...
  for (const side of ["Left", "Right"]) {
    ...
    aim(thigh, shin, origin.clone().add(new Vec32(0, -0.04, upper)));
    aim(shin, foot, shin.getPosition().clone().add(new Vec32(0, -lower, 0.035)));
  }
  arms(0.67, 0.27, 0.12);
...
import { Color, Entity as Entity3, StandardMaterial } from "playcanvas";
...
import { AnimStateGraph, Mat4, Vec3 as Vec33, math, SEMANTIC_BLENDINDICES, SEMANTIC_BLENDWEIGHT } from "playcanvas";
var CharacterAnimator = class {
  ...
  geometrySnapshot() {
    ...
    for (const render of this.model?.findComponents("render") ?? []) for (const instance of render.meshInstances) {
      ...
      const source = new Vec33(), transformed = new Vec33(), world = new Vec33();
...
import { AnimCurve as AnimCurve3, AnimData as AnimData3, AnimTrack as AnimTrack3, Quat as Quat3, Vec3 as Vec34, INTERPOLATION_LINEAR as INTERPOLATION_LINEAR3 } from "playcanvas";
function bedEntryTrack(model, source, sleep) {
  ...
  for (let stage = 0; stage < 4; stage++) {
    ...
    for (const [s, sign] of [["Left", 1], ["Right", -1]]) {
      aim(bone(s, "arm"), bone(s, "fore"), stage === 1 ? new Vec34(sign * 0.2, 0.25, 0.8) : stage === 2 ? new Vec34(sign * 0.25, -0.3, 0.7) : new Vec34(sign * 0.12, -1, 0.06));
      aim(bone(s, "fore"), bone(s, "hand"), stage === 1 ? new Vec34(0, 0.2, 0.8) : new Vec34(0, -0.7, 0.35));
      const tuck = stage >= 2 ? 1 : stage === 1 && s === "Left" ? 0.6 : 0;
      aim(bone(s, "thigh"), bone(s, "shin"), new Vec34(sign * 0.04, -1 + tuck, tuck));
      aim(bone(s, "shin"), bone(s, "foot"), new Vec34(0, -1, stage >= 2 ? -0.15 : 0));
    }
    frames.push(read());
  }
...
function sleepingTrack(model, source, motion) {
  ...
  for (let f = 0; f <= 24; f++) {
    ...
    for (const [side, knee, elbow] of [["Left", motion.leftKnee, motion.leftElbow], ["Right", motion.rightKnee, motion.rightElbow]]) {
      ...
      aim(bone(side, "thigh"), bone(side, "shin"), new Vec34(sign * 0.05, -Math.cos(rad / 2), Math.sin(rad / 2)));
      aim(bone(side, "shin"), bone(side, "foot"), new Vec34(0, -Math.cos(rad / 2), -Math.sin(rad / 2)));
      aim(bone(side, "arm"), bone(side, "fore"), new Vec34(sign * 0.15, -1, 0.05));
      aim(bone(side, "fore"), bone(side, "hand"), new Vec34(-sign * 0.12, -1, Math.sin(elbow * Math.PI / 180) * 0.4));
    }
...
function createCharacter(app) {
  const player = new Entity6("Arianna", app);
  app.root.addChild(player);
  player.setPosition(0, 0.09, 0.9);
  const visual = new Entity6("Character visual pivot", app);
  player.addChild(visual);
  visual.setLocalEulerAngles(0, 30, 0);
  const placeholder = new Entity6("Temporary capsule", app);
...
async function loadArianna(app, character, resolveAsset = (path) => `${"/"}${path}`) {
  ...
  const alignment = new Entity6("GLB alignment", app);
...
import { Keyboard, Vec2, Vec3 as Vec35, KEY_A, KEY_D, KEY_S, KEY_W, KEY_UP, KEY_DOWN, KEY_LEFT, KEY_RIGHT } from "playcanvas";
var PlayerController = class {
  ...
  approachProp(point, arrived, cancelled) {
    ...
    const clear = (p) => {
      ...
      for (let i = 1; i <= count; i++) if (!free(new Vec35().lerp(start, p, i / count))) return false;
      return true;
    };
    for (let radius = 0.32; radius <= 1.8; radius += 0.06) for (let angle = 0; angle < Math.PI * 2; angle += Math.PI / 24) {
      const p = new Vec35(point.x + Math.sin(angle) * radius, start.y, point.z + Math.cos(angle) * radius);
...
import { BoundingBox as BoundingBox2, Entity as Entity8, Mat4 as Mat42, Vec3 as Vec36 } from "playcanvas";
var CarrySystem = class {
  socket;
  item = null;
  constructor(app, visual) {
    this.socket = new Entity8("Carry socket", app);
  ...
  pickUp(item) {
    ...
    const center = item.carryGrip ? new Vec36(...item.carryGrip) : first ? new Vec36() : new Mat42().copy(item.entity.getWorldTransform()).invert().transformPoint(bounds.center);
...
import { Entity as Entity9, Color as Color2, PROJECTION_ORTHOGRAPHIC, TONEMAP_LINEAR, Vec3 as Vec37 } from "playcanvas";
...
var IsometricCamera = class {
  ...
  constructor(app, existing) {
    this.entity = existing ?? new Entity9("Following isometric camera", app);
    if (!this.entity.camera) this.entity.addComponent("camera", {
      projection: PROJECTION_ORTHOGRAPHIC,
      orthoHeight: 7,
      nearClip: 0.1,
      farClip: 60,
      clearColor: new Color2().fromString("#ede6f4")
    });
    this.entity.setPosition(6, 14, 18.9);
    this.entity.lookAt(new Vec37(0, 0.8, 0.9));
  ...
  follow(player, dt) {
    ...
    this.entity.lookAt(new Vec37(this.offset.x, 0.8, 0.9 + this.offset.z));
  }
  reset() {
...
async function startPilot(app) {
  ...
  const bounds = find("Room bounds"), boundsBox = new BoundingBox3();
  boundsBox.setFromTransformedAabb(new BoundingBox3(new Vec38(), new Vec38(0.5, 0.5, 0.5)), bounds.getWorldTransform());
  const room = { root, obstacles: [], halfWidth: 3.3, halfDepth: 3.6, walkable: [{ minX: boundsBox.center.x - boundsBox.halfExtents.x, maxX: boundsBox.center.x + boundsBox.halfExtents.x, minZ: boundsBox.center.z - boundsBox.halfExtents.z, maxZ: boundsBox.center.z + boundsBox.halfExtents.z }] };
  const colliderNodes = root.findByTag("pilot.collider");
  const refreshBounds = () => {
    room.obstacles = colliderNodes.map((e) => {
      const b = new BoundingBox3();
      b.setFromTransformedAabb(new BoundingBox3(new Vec38(), new Vec38(0.5, 0.5, 0.5)), e.getWorldTransform());
      return b;
    });
  };
  refreshBounds();
  ...
  camera.entity.camera.clearColor = new Color3().fromString("#ede6f4");
  ...
  mission.configure([{ id: "book", name: "Book", icon: "\u{1F4D8}" }], false);
  mission.start(performance.now());
  const bookHome = book.getPosition().clone();
  const item = { id: "book", name: "Book", icon: "\u{1F4D8}", entity: book, home: bookHome.toArray() };
  const pickup = { id: "pickup-book", name: "Book", icon: "\u{1F4D8}", kind: "pickup", item: "book", anchor: bookHome.clone(), marker: bookHome.clone().add(new Vec38(0, 0.6, 0)), range: 0.85 };
  const drop = { id: "bookshelf", name: "Bookshelf", icon: "\u{1F4D8}", kind: "place", item: "book", task: "book", anchor: destination.getPosition().clone(), marker: marker.getPosition().clone(), range: 1.05, placement: placement.getPosition().toArray() };
  const interactions = new InteractionSystem([pickup, drop]);
  ...
  app.on("update", (elapsed) => {
    const dt = document.hidden ? 0 : Math.min(elapsed, 0.04), now = performance.now();
